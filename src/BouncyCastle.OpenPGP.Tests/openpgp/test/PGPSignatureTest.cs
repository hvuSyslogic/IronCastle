...
namespace org.bouncycastle.openpgp.test
{
	...
	public class PGPSignatureTest : SimpleTest
	{
		...
		public override void performTest()
		{
			...
			try
			{
				testSig(PublicKeyAlgorithmTags_Fields.DSA, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey);
			...
			try
			{
				testSigV3(PublicKeyAlgorithmTags_Fields.DSA, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey);
			...
			sGen = new PGPSignatureGenerator((new JcaPGPContentSignerBuilder(PublicKeyAlgorithmTags_Fields.DSA, HashAlgorithmTags_Fields.SHA1)).setProvider("BC"));
			...
			if (criticalHashed[0] != SignatureSubpacketTags_Fields.SIGNER_USER_ID)
			{
			...
			hashedGen.setSignatureCreationTime(false, new DateTime(0L));
			...
			if (!hashedPcks.hasSubpacket(SignatureSubpacketTags_Fields.CREATION_TIME))
			{
				fail("hasSubpacket test for creation time failed");
			}

			if (!hashedPcks.getSignatureCreationTime().Equals(new DateTime(0L)))
			{
			...
			if (!string.ReferenceEquals(hashedPcks.getSignerUserID(), null))
			{
			...
			// general signatures
			//
			testSig(PublicKeyAlgorithmTags_Fields.RSA_GENERAL, HashAlgorithmTags_Fields.SHA256, secretKey.getPublicKey(), pgpPrivKey);
			testSig(PublicKeyAlgorithmTags_Fields.RSA_GENERAL, HashAlgorithmTags_Fields.SHA384, secretKey.getPublicKey(), pgpPrivKey);
			testSig(PublicKeyAlgorithmTags_Fields.RSA_GENERAL, HashAlgorithmTags_Fields.SHA512, secretKey.getPublicKey(), pgpPrivKey);
			testSigV3(PublicKeyAlgorithmTags_Fields.RSA_GENERAL, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey);
			testTextSig(PublicKeyAlgorithmTags_Fields.RSA_GENERAL, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey, TEST_DATA_WITH_CRLF, TEST_DATA_WITH_CRLF);
			testTextSig(PublicKeyAlgorithmTags_Fields.RSA_GENERAL, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey, TEST_DATA, TEST_DATA_WITH_CRLF);
			testTextSigV3(PublicKeyAlgorithmTags_Fields.RSA_GENERAL, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey, TEST_DATA_WITH_CRLF, TEST_DATA_WITH_CRLF);
			testTextSigV3(PublicKeyAlgorithmTags_Fields.RSA_GENERAL, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey, TEST_DATA, TEST_DATA_WITH_CRLF);
			...
			try
			{
				testSig(PublicKeyAlgorithmTags_Fields.RSA_GENERAL, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey);
			...
			testSig(PublicKeyAlgorithmTags_Fields.DSA, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey);
			testSigV3(PublicKeyAlgorithmTags_Fields.DSA, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey);
			testTextSig(PublicKeyAlgorithmTags_Fields.DSA, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey, TEST_DATA_WITH_CRLF, TEST_DATA_WITH_CRLF);
			testTextSig(PublicKeyAlgorithmTags_Fields.DSA, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey, TEST_DATA, TEST_DATA_WITH_CRLF);
			testTextSigV3(PublicKeyAlgorithmTags_Fields.DSA, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey, TEST_DATA_WITH_CRLF, TEST_DATA_WITH_CRLF);
			testTextSigV3(PublicKeyAlgorithmTags_Fields.DSA, HashAlgorithmTags_Fields.SHA1, secretKey.getPublicKey(), pgpPrivKey, TEST_DATA, TEST_DATA_WITH_CRLF);

			// special cases
			//
			testMissingSubpackets(nullPacketsSubKeyBinding);

			testMissingSubpackets(generateV3BinarySig(pgpPrivKey, PublicKeyAlgorithmTags_Fields.DSA, HashAlgorithmTags_Fields.SHA1));
		...
		private void testSignatureTarget()
		{
			...
			sGen.setSignatureTarget(true, PublicKeyAlgorithmTags_Fields.ECDSA, HashAlgorithmTags_Fields.SHA1, hash);

			PGPSignatureSubpacketVector sVec = sGen.generate();

			isTrue("no sig target", sVec.hasSubpacket(SignatureSubpacketTags_Fields.SIGNATURE_TARGET));

			SignatureTarget sigTarg = sVec.getSignatureTarget();

			isTrue("wrong critical", sigTarg.isCritical());
			isTrue("wrong key alg", PublicKeyAlgorithmTags_Fields.ECDSA == sigTarg.getPublicKeyAlgorithm());
			isTrue("wrong hash alg", HashAlgorithmTags_Fields.SHA1 == sigTarg.getHashAlgorithm());
			isTrue("wrong hash data", Arrays.areEqual(hash, sigTarg.getHashData()));

			sGen = new PGPSignatureSubpacketGenerator();

			sGen.setSignatureTarget(false, PublicKeyAlgorithmTags_Fields.RSA_SIGN, HashAlgorithmTags_Fields.SHA256, hash);
			...
			isTrue("wrong critical", !sigTarg.isCritical());
			isTrue("wrong key alg", PublicKeyAlgorithmTags_Fields.RSA_SIGN == sigTarg.getPublicKeyAlgorithm());
			isTrue("wrong hash alg", HashAlgorithmTags_Fields.SHA256 == sigTarg.getHashAlgorithm());
			isTrue("wrong hash data", Arrays.areEqual(hash, sigTarg.getHashData()));
		}
		...
		private void preferredAlgorithmCheck(string type, int[] expected, int[] prefAlgs)
		{
			if (expected == null)
			{
				if (prefAlgs != null)
		...
		private void testSig(int encAlgorithm, int hashAlgorithm, PGPPublicKey pubKey, PGPPrivateKey privKey)
		{
			...
			OutputStream lOut = lGen.open(new UncloseableOutputStream(bOut), PGPLiteralData.BINARY, "_CONSOLE", TEST_DATA.Length * 2, DateTime.Now);
		...
		private void testTextSig(int encAlgorithm, int hashAlgorithm, PGPPublicKey pubKey, PGPPrivateKey privKey, byte[] data, byte[] canonicalData)
		{
			PGPSignatureGenerator sGen = new PGPSignatureGenerator((new JcaPGPContentSignerBuilder(encAlgorithm, HashAlgorithmTags_Fields.SHA1)).setProvider("BC"));
			ByteArrayOutputStream bOut = new ByteArrayOutputStream();
			...
			PGPLiteralDataGenerator lGen = new PGPLiteralDataGenerator();
			OutputStream lOut = lGen.open(new UncloseableOutputStream(bOut), PGPLiteralData.TEXT, "_CONSOLE", data.Length * 2, creationTime);
			...
			if (sig.getCreationTime().Ticks == 0)
			{
		...
		private void testSigV3(int encAlgorithm, int hashAlgorithm, PGPPublicKey pubKey, PGPPrivateKey privKey)
		{
		...
		private void testTextSigV3(int encAlgorithm, int hashAlgorithm, PGPPublicKey pubKey, PGPPrivateKey privKey, byte[] data, byte[] canonicalData)
		{
			PGPV3SignatureGenerator sGen = new PGPV3SignatureGenerator((new JcaPGPContentSignerBuilder(encAlgorithm, HashAlgorithmTags_Fields.SHA1)).setProvider("BC"));
			ByteArrayOutputStream bOut = new ByteArrayOutputStream();
			ByteArrayInputStream testIn = new ByteArrayInputStream(data);
			...
			PGPLiteralDataGenerator lGen = new PGPLiteralDataGenerator();
			OutputStream lOut = lGen.open(new UncloseableOutputStream(bOut), PGPLiteralData.TEXT, "_CONSOLE", data.Length * 2, DateTime.Now);
		...
		private void verifySignature(byte[] encodedSig, int hashAlgorithm, PGPPublicKey pubKey, byte[] original)
		{
			...
			DateTime creationTime = sig.getCreationTime();
			DateTime now = DateTime.Now;

			// Check creationTime is recent
			if (creationTime > now || creationTime < new DateTime(now.Ticks - 10 * 60 * 1000))
			{
