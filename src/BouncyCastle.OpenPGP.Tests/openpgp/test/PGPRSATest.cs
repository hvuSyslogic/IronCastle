...
namespace org.bouncycastle.openpgp.test
{
	...
	public class PGPRSATest : SimpleTest
	{
		...
		private void mixedTest(PGPPrivateKey pgpPrivKey, PGPPublicKey pgpPubKey)
		{
			...
			OutputStream lOut = lGen.open(bOut, PGPLiteralData.BINARY, PGPLiteralData.CONSOLE, text.Length, DateTime.Now);
			...
			PGPEncryptedDataGenerator encGen = new PGPEncryptedDataGenerator((new JcePGPDataEncryptorBuilder(SymmetricKeyAlgorithmTags_Fields.AES_128)).setWithIntegrityPacket(true).setSecureRandom(new SecureRandom()).setProvider("BC"));
		...
		private void embeddedJpegTest()
		{
			...
			PGPSignatureGenerator sGen = new PGPSignatureGenerator((new JcaPGPContentSignerBuilder(PublicKeyAlgorithmTags_Fields.RSA_GENERAL, HashAlgorithmTags_Fields.SHA1)).setProvider("BC"));
		...
		private void sigsubpacketTest()
		{
			...
			int[] encAlgs = new int[] {SymmetricKeyAlgorithmTags_Fields.AES_256, SymmetricKeyAlgorithmTags_Fields.AES_192, SymmetricKeyAlgorithmTags_Fields.TRIPLE_DES};
			svg.setPreferredSymmetricAlgorithms(true, encAlgs);
			int[] hashAlgs = new int[] {HashAlgorithmTags_Fields.SHA1, HashAlgorithmTags_Fields.SHA512, HashAlgorithmTags_Fields.SHA384, HashAlgorithmTags_Fields.SHA256, HashAlgorithmTags_Fields.RIPEMD160};
			svg.setPreferredHashAlgorithms(true, hashAlgs);
			int[] comprAlgs = new int[] {CompressionAlgorithmTags_Fields.ZLIB, CompressionAlgorithmTags_Fields.BZIP2, CompressionAlgorithmTags_Fields.ZIP};
			...
			PGPKeyRingGenerator keyRingGen = new PGPKeyRingGenerator(PGPSignature.POSITIVE_CERTIFICATION, sgnKeyPair, identity, sha1Calc, hashedPcks, unhashedPcks, new JcaPGPContentSignerBuilder(PGPPublicKey.RSA_SIGN, HashAlgorithmTags_Fields.SHA1), (new JcePBESecretKeyEncryptorBuilder(PGPEncryptedData.AES_256)).setProvider("BC").build(passPhrase));
		...
		private void multipleExpiryTest()
		{
			...
			PGPDigestCalculator sha1Calc = (new JcaPGPDigestCalculatorProviderBuilder()).build().get(HashAlgorithmTags_Fields.SHA1);
			PGPKeyRingGenerator keyRingGen = new PGPKeyRingGenerator(PGPSignature.POSITIVE_CERTIFICATION, sgnKeyPair, identity, sha1Calc, hashedPcks, unhashedPcks, new JcaPGPContentSignerBuilder(sgnKeyPair.getPublicKey().getAlgorithm(), HashAlgorithmTags_Fields.SHA1), (new JcePBESecretKeyEncryptorBuilder(PGPEncryptedData.AES_256)).setProvider("BC").build(passPhrase));
			...
			for (Iterator it = keyRing.getPublicKeys(); it.hasNext();)
			{
				...
				PGPSignatureGenerator keySigGen = new PGPSignatureGenerator((new JcaPGPContentSignerBuilder(PGPPublicKey.RSA_SIGN, HashAlgorithmTags_Fields.SHA1)).setProvider("BC"));
		...
		public override void performTest()
		{
			...
			//
			byte[] shortText = new byte[] {(byte)'h', (byte)'e', (byte)'l', (byte)'l', (byte)'o'};

			ByteArrayOutputStream cbOut = new ByteArrayOutputStream();
			PGPEncryptedDataGenerator cPk = new PGPEncryptedDataGenerator((new JcePGPDataEncryptorBuilder(SymmetricKeyAlgorithmTags_Fields.CAST5)).setSecureRandom(new SecureRandom()).setProvider("BC"));
			...
			if (encP.getSymmetricAlgorithm(dataDecryptorFactory) != SymmetricKeyAlgorithmTags_Fields.CAST5)
			{
			...
			PGPSecretKey secretKey = new PGPSecretKey(PGPSignature.DEFAULT_CERTIFICATION, new JcaPGPKeyPair(PublicKeyAlgorithmTags_Fields.RSA_GENERAL, kp, DateTime.Now), "fred", null, null, (new JcaPGPContentSignerBuilder(PublicKeyAlgorithmTags_Fields.RSA_SIGN, HashAlgorithmTags_Fields.SHA1)).setProvider("BC"), (new JcePBESecretKeyEncryptorBuilder(SymmetricKeyAlgorithmTags_Fields.CAST5)).build(passPhrase));
			...
			// use of PGPKeyPair
			//
			PGPKeyPair pgpKp = new JcaPGPKeyPair(PGPPublicKey.RSA_GENERAL, kp, DateTime.Now);
			...
			secretKey = new PGPSecretKey(PGPSignature.DEFAULT_CERTIFICATION, new JcaPGPKeyPair(PublicKeyAlgorithmTags_Fields.RSA_GENERAL, kp, DateTime.Now), "fred", null, null, (new JcaPGPContentSignerBuilder(PublicKeyAlgorithmTags_Fields.RSA_SIGN, HashAlgorithmTags_Fields.SHA1)).setProvider("BC"), (new JcePBESecretKeyEncryptorBuilder(SymmetricKeyAlgorithmTags_Fields.AES_256)).build(passPhrase));
			...
			sGen = new PGPSignatureGenerator((new JcaPGPContentSignerBuilder(PublicKeyAlgorithmTags_Fields.RSA_GENERAL, HashAlgorithmTags_Fields.SHA1)).setProvider("BC"));
			...
			lOut = lGen.open(new UncloseableOutputStream(bcOut), PGPLiteralData.BINARY, "_CONSOLE", data.GetBytes().length, testDate);
		...
		private void testExpiry(byte[] encodedRing, int masterDays, int subKeyDays)
		{
