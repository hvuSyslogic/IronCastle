namespace org.bouncycastle.openpgp.test
{
	...
	public class PGPNoPrivateKeyTest : SimpleTest
	{
		...
		public override void performTest()
		{
			...
			try
			{
				PGPSecretKey pgpChangedKey = PGPSecretKey.copyWithNewPassword(pgpSecKey, (new JcePBESecretKeyDecryptorBuilder((new JcaPGPDigestCalculatorProviderBuilder()).setProvider(BOUNCY_CASTLE_PROVIDER_NAME).build())).setProvider(BOUNCY_CASTLE_PROVIDER_NAME).build(pgpOldPass.ToCharArray()), (new JcePBESecretKeyEncryptorBuilder(pgpSecKey.getKeyEncryptionAlgorithm())).build(pgpNewPass.ToCharArray()));
			}
