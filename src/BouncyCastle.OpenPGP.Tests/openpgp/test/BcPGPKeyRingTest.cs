...
namespace org.bouncycastle.openpgp.test
{
	...
	using BcPGPPublicKeyRingCollection = org.bouncycastle.openpgp.bc.BcPGPPublicKeyRingCollection;
	using BcPGPSecretKeyRingCollection = org.bouncycastle.openpgp.bc.BcPGPSecretKeyRingCollection;
	...
	public class BcPGPKeyRingTest : SimpleTest
	{
		...
		private void rewrapTest()
		{
			...
			BcPGPSecretKeyRingCollection privRings = new BcPGPSecretKeyRingCollection(new ByteArrayInputStream(rewrapKey));
			...
			if (rIt.hasNext())
			{
				...
				while (it.hasNext())
				{
					...
					pgpKey = PGPSecretKey.copyWithNewPassword(pgpKey, (new BcPBESecretKeyDecryptorBuilder(new BcPGPDigestCalculatorProvider())).build(rewrapPass), null);
		...
		{
			BufferedInputStream bin = new BufferedInputStream(this.GetType().getResourceAsStream("longSigSubPack.asc"));
			PGPPublicKeyRing pkr = new PGPPublicKeyRing(PGPUtil.getDecoderStream(bin), new BcKeyFingerprintCalculator());
			bin.close();
