...
namespace org.bouncycastle.openpgp.test
{
	...
	public class PGPPacketTest : SimpleTest
	{
		private static int MAX = 32000;

		private void readBackTest(PGPLiteralDataGenerator generator)
		{
		...
		private void bufferTest(PGPLiteralDataGenerator generator, byte[] buf, int i)
		{
			ByteArrayOutputStream bOut = new ByteArrayOutputStream();
			OutputStream @out = generator.open(new UncloseableOutputStream(bOut), PGPLiteralData.BINARY, PGPLiteralData.CONSOLE, i, DateTime.Now);
		...
		private void iteratorTest()
		{
			...
			for (int i = 0; i < packets; i++)
			{
				...
				OutputStream @out = generator.open(new UncloseableOutputStream(msg), PGPLiteralData.BINARY, PGPLiteralData.CONSOLE, packetLength, DateTime.Now);
