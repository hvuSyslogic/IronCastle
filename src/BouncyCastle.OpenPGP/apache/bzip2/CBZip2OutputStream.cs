...
namespace org.bouncycastle.apache.bzip2
{
	...
	public class CBZip2OutputStream : OutputStream, BZip2Constants
	{
		...
		*/
		...
		protected internal static void hbMakeCodeLengths(char[] len, int[] freq, int alphaSize, int maxLen)
		{
			...
			*/
			...
			int[] heap = new int[BZip2Constants_Fields.MAX_ALPHA_SIZE + 2];
			int[] weight = new int[BZip2Constants_Fields.MAX_ALPHA_SIZE * 2];
			int[] parent = new int[BZip2Constants_Fields.MAX_ALPHA_SIZE * 2];
			...
			while (true)
			{
				...
				if (!(nHeap < (BZip2Constants_Fields.MAX_ALPHA_SIZE + 2)))
				{
					panic();
				}
				...
				while (nHeap > 1)
				{
					...
					weight[nNodes] = ((weight[n1] & unchecked((int)0xffffff00)) + (weight[n2] & unchecked((int)0xffffff00))) | (1 + (((weight[n1] & 0x000000ff) > (weight[n2] & 0x000000ff)) ? (weight[n1] & 0x000000ff) : (weight[n2] & 0x000000ff)));
				...
				if (!(nNodes < (BZip2Constants_Fields.MAX_ALPHA_SIZE * 2)))
				{
					panic();
				}
		...
		*/
		...
		public CBZip2OutputStream(OutputStream inStream) : this(inStream, 9)
		{
		}
		...
		/// 
		/// </summary>
		public virtual void write(int bv)
		{
			int b = (256 + bv) % 256;
			if (currentChar != -1)
			{
...
//			base.finalize();
		...
		private void initBlock()
		{
			...
			allowableBlockSize = BZip2Constants_Fields.baseBlockSize * blockSize100k - 20;
		}
...
//ORIGINAL LINE: char[][] len = new char[BZip2Constants_Fields.N_GROUPS][BZip2Constants_Fields.MAX_ALPHA_SIZE];
			...
			alphaSize = nInUse + 2;
			for (t = 0; t < BZip2Constants_Fields.N_GROUPS; t++)
			{
...
//JAVA TO C# CONVERTER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
//ORIGINAL LINE: int[][] rfreq = new int[BZip2Constants_Fields.N_GROUPS][BZip2Constants_Fields.MAX_ALPHA_SIZE];
			int[][] rfreq = RectangularArrays.ReturnRectangularIntArray(BZip2Constants_Fields.N_GROUPS, BZip2Constants_Fields.MAX_ALPHA_SIZE);
			int[] fave = new int[BZip2Constants_Fields.N_GROUPS];
			short[] cost = new short[BZip2Constants_Fields.N_GROUPS];
			/*
			  Iterate up to N_ITERS times to improve the tables.
			*/
			for (iter = 0; iter < BZip2Constants_Fields.N_ITERS; iter++)
			{
				for (t = 0; t < nGroups; t++)
				{
					fave[t] = 0;
				...
				while (true)
				{
					...
					ge = gs + BZip2Constants_Fields.G_SIZE - 1;
					if (ge >= nMTF)
					{
						ge = nMTF - 1;
					}
			...
			if (!(nSelectors < 32768 && nSelectors <= (2 + (900000 / BZip2Constants_Fields.G_SIZE))))
			{
				panic();
			}
...
//JAVA TO C# CONVERTER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
//ORIGINAL LINE: int[][] code = new int[BZip2Constants_Fields.N_GROUPS][BZip2Constants_Fields.MAX_ALPHA_SIZE];
			int[][] code = RectangularArrays.ReturnRectangularIntArray(BZip2Constants_Fields.N_GROUPS, BZip2Constants_Fields.MAX_ALPHA_SIZE);
	...
	//        nBytes = bytesOut;
			...
			for (i = 0; i < nSelectors; i++)
			{
				for (j = 0; (char)j < selectorMtf[i]; j++)
				{
					bsW(1, 1);
				}
				bsW(1, 0);
	...
	//        nBytes = bytesOut;
		...
		private void mainSort()
		{
			...
			*/
			...
			for (i = 0; i < BZip2Constants_Fields.NUM_OVERSHOOT_BYTES; i++)
			{
				block[last + i + 2] = block[(i % (last + 1)) + 1];
			}
			for (i = 0; i <= last + BZip2Constants_Fields.NUM_OVERSHOOT_BYTES; i++)
			{
				quadrant[i] = 0;
			}
		...
		*/
		...
		{
			int n = BZip2Constants_Fields.baseBlockSize * blockSize100k;
			block = new char[(n + 1 + BZip2Constants_Fields.NUM_OVERSHOOT_BYTES)];
			quadrant = new int[(n + BZip2Constants_Fields.NUM_OVERSHOOT_BYTES)];
			zptr = new int[n];
			ftab = new int[65537];
				...
				else
				{
					if (zPend > 0)
					{
						...
						while (true)
						{
							...
							{
							case 0:
								szptr[wr] = (short) BZip2Constants_Fields.RUNA;
								wr++;
								mtfFreq[BZip2Constants_Fields.RUNA]++;
								break;
							case 1:
								szptr[wr] = (short) BZip2Constants_Fields.RUNB;
								wr++;
