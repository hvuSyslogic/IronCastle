...
namespace org.bouncycastle.openpgp.examples
{
	...
	{
		private static void exportKeyPair(OutputStream secretOut, OutputStream publicOut, KeyPair dsaKp, KeyPair elgKp, string identity, char[] passPhrase, bool armor)
		{
			if (armor)
			{
			...
			PGPKeyPair dsaKeyPair = new JcaPGPKeyPair(PGPPublicKey.DSA, dsaKp, DateTime.Now);
			PGPKeyPair elgKeyPair = new JcaPGPKeyPair(PGPPublicKey.ELGAMAL_ENCRYPT, elgKp, DateTime.Now);
			PGPDigestCalculator sha1Calc = (new JcaPGPDigestCalculatorProviderBuilder()).build().get(HashAlgorithmTags_Fields.SHA1);
			PGPKeyRingGenerator keyRingGen = new PGPKeyRingGenerator(PGPSignature.POSITIVE_CERTIFICATION, dsaKeyPair, identity, sha1Calc, null, null, new JcaPGPContentSignerBuilder(dsaKeyPair.getPublicKey().getAlgorithm(), HashAlgorithmTags_Fields.SHA1), (new JcePBESecretKeyEncryptorBuilder(PGPEncryptedData.AES_256, sha1Calc)).setProvider("BC").build(passPhrase));
		...
		public static void Main(string[] args)
		{
			...
			if (args.Length < 2)
			{
				JavaSystem.@out.println("DSAElGamalKeyRingGenerator [-a] identity passPhrase");
				System.exit(0);
			}
