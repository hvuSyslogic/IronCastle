...
namespace org.bouncycastle.pqc.crypto.test
{
	...
	/// 
	/// </summary>
	public class XMSSUtilTest : TestCase
	{

		public virtual void testLog2()
		{
		...
		public virtual void testIntToBytesBigEndian()
		{
		...
		public virtual void testLongToBytesBigEndian()
		{
		...
		public virtual void testLongToBytesBigEndianOffsetException()
		{
			try
			{
				byte[] @in = new byte[8];
		...
		public virtual void testLongToBytesBigEndianOffset()
		{
			byte[] @in = new byte[32];
		...
		public virtual void testBytesToIntBigEndianException()
		{
			byte[] @in = new byte[4];
			try
			{
		...
		public virtual void testBytesToIntBigEndian()
		{
			byte[] in1 = new byte[] {0x00, unchecked((byte)0xff), 0x00, unchecked((byte)0xff)};
			int @out = Pack.bigEndianToInt(in1, 0);
			assertEquals(16711935, @out);
			byte[] in2 = new byte[] {unchecked((byte)0xab), unchecked((byte)0xcd), unchecked((byte)0xef), unchecked((byte)0xaa)};
			@out = Pack.bigEndianToInt(in2, 0);
			assertEquals("2882400170", (new BigInteger(1, in2)).ToString());
			byte[] in3 = new byte[100];
			Arrays.fill(in3, unchecked((byte)0xaa));
			for (int i = 35; i < 39; i++)
			{
				in3[i] = unchecked((byte)0xff);
			}
			@out = Pack.bigEndianToInt(in3, 35);
			assertEquals(new BigInteger("4294967295"), BigInteger.valueOf(@out & 0xffffffffL));
		}

		public virtual void testBytesToLongBigEndianException()
		{
			byte[] @in = new byte[10];
			try
			{
		...
		public virtual void testBytesToLongBigEndian()
		{
			byte[] in1 = new byte[] {0x00, unchecked((byte)0xff), 0x00, unchecked((byte)0xff), 0x00, unchecked((byte)0xff), 0x00, unchecked((byte)0xff)};
			long @out = Pack.bigEndianToLong(in1, 0);
			assertEquals(71777214294589695L, @out);
			byte[] in2 = new byte[] {unchecked((byte)0xff), unchecked((byte)0xff), unchecked((byte)0xff), unchecked((byte)0xff), unchecked((byte)0xff), unchecked((byte)0xff), unchecked((byte)0xff), unchecked((byte)0xff)};
			@out = Pack.bigEndianToLong(in2, 0);
			assertEquals("18446744073709551615", (new BigInteger(1, in2)).ToString());
		}

		public virtual void testCalculateTau()
		{
			int height = 10;
