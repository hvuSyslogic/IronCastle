namespace org.bouncycastle.util.encoders.test
{
	...
	public class UTF8Test : SimpleTest
	{
		...
		private void testBadSuffix_E0()
		{
			...
			utf8[0] = unchecked((byte)0xE0);
			utf8[1] = randomSuffix();
			utf8[2] = randomSuffix();
			utf16[0] = (char)0;

			utf8[1] &= unchecked((byte)0x9F);
		...
		private void testValid4()
		{
			...
			for (int i = 0x00; i < 0x05; ++i)
			{
				...
				isEquals("testValid4.Value", ((utf16[0] - 0xD7C0) << 10) | (utf16[1] - 0xDC00), (i << 18) | ((utf8[1] & 0x3F) << 12) | ((utf8[2] & 0x3F) << 6) | (utf8[3] & 0x3F));
			}
		}
		...
		private byte randomInRange(byte lo, byte hi)
		{
			...
			return (byte)(lo + (((int)((uint)R.nextInt() >> 1)) % d));
		}
		...
		private byte randomPrefix()
		{
			...
			{
			case 2:
				return randomInRange(unchecked((byte)0xC2), unchecked((byte)0xE0));
			case 3:
				return randomInRange(unchecked((byte)0xE0), unchecked((byte)0xF0));
			case 4:
				return randomInRange(unchecked((byte)0xF0), unchecked((byte)0xF6));
			default:
		...
		{
			return unchecked((byte)(((int)((uint)R.nextInt() >> 26)) | 0x80));
		}
	}
