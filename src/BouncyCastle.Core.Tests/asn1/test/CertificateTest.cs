namespace org.bouncycastle.asn1.test
{
	...
	public class CertificateTest : SimpleTest
	{
		//
		// server.crt
		//
	   ...
	   internal string[] subjects = new string[] {"C=AU,ST=Victoria,L=South Melbourne,O=Connect 4 Pty Ltd,OU=Webserver Team,CN=www2.connect4.com.au,E=webmaster@connect4.com.au", "C=AU,ST=Victoria,L=South Melbourne,O=Connect 4 Pty Ltd,OU=Certificate Authority,CN=Connect 4 CA,E=webmaster@connect4.com.au", "C=AU,ST=QLD,CN=SSLeay/rsa test cert", "C=US,O=National Aeronautics and Space Administration,SERIALNUMBER=16+CN=Steve Schoch", "E=cooke@issl.atl.hp.com,C=US,OU=Hewlett Packard Company (ISSL),CN=Paul A. Cooke", "O=Sun Microsystems Inc,CN=store.sun.com", "unstructuredAddress=192.168.1.33,unstructuredName=pixfirewall.ciscopix.com,CN=pixfirewall.ciscopix.com"};
		...
		public virtual void checkCertificate(int id, byte[] cert)
		{
	...
	//        String dump = ASN1Dump.dumpAsString(seq);
			...
			if (tbsCert.getVersionNumber() == 3)
			{
				...
				if (ext != null)
				{
					...
					while (en.hasMoreElements())
					{
						...
						{
							//JavaSystem.@out.println(oid.getId());
						}
					}
				}
			}
		...
		public virtual void checkAttributeCertificate(int id, byte[] cert)
		{
	...
	//        String dump = ASN1Dump.dumpAsString(seq);
			...
			// Version
			if (!(acInfo.getVersion().Equals(new ASN1Integer(1))) && (!(acInfo.getVersion().Equals(new ASN1Integer(2)))))
			{
		...
		public virtual void checkV1AttributeCertificate(int id, byte[] cert)
		{
			ByteArrayInputStream bIn;
			ASN1InputStream aIn;
