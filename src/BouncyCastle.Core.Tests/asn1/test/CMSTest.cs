	...
	public class CMSTest : Test
	{
		...
		private TestResult compressionTest()
		{
			try
			{
				...
				info = new ContentInfo(CMSObjectIdentifiers_Fields.compressedData, data);
		...
		private TestResult envelopedTest()
		{
			try
			{
				...
				info = new ContentInfo(CMSObjectIdentifiers_Fields.envelopedData, envData);
				...
				EnvelopedDataParser ed = new EnvelopedDataParser((ASN1SequenceParser)ci.getContent(BERTags_Fields.SEQUENCE));
				ed.getVersion();
				...
				InputStream dataIn = ((ASN1OctetStringParser)eci.getEncryptedContent(BERTags_Fields.OCTET_STRING)).getOctetStream();
		...
		private TestResult signedTest()
		{
			try
			{
				...
				info = new ContentInfo(CMSObjectIdentifiers_Fields.signedData, sData);
