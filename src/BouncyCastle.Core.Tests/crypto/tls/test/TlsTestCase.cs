...
namespace org.bouncycastle.crypto.tls.test
{
	...
	public class TlsTestCase : TestCase
	{
		...
		public TlsTestCase(string name) : base(name)
		{

			this.config = null;
		}

		public TlsTestCase(TlsTestConfig config, string name) : base(name)
		{
		...
		public virtual void runTest()
		{
			...
			ServerThread serverThread = new ServerThread(this, serverProtocol, serverImpl);
			serverThread.start();
		...
		{
			private readonly TlsTestCase outerInstance;

			protected internal readonly TlsTestServerProtocol serverProtocol;
			protected internal readonly TlsTestServerImpl serverImpl;

			internal bool canExit = false;
			internal Exception caught = null;

			public ServerThread(TlsTestCase outerInstance, TlsTestServerProtocol serverProtocol, TlsTestServerImpl serverImpl)
			{
