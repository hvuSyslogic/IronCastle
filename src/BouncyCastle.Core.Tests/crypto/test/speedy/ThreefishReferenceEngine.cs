namespace org.bouncycastle.crypto.test.speedy
{
	...
	public class ThreefishReferenceEngine : BlockCipher
	{
		private bool InstanceFieldsInitialized = false;

		public ThreefishReferenceEngine()
		{
		...
		/// </summary>
		private const int TWEAK_SIZE = 16;

		private static long C_240 = 0x1BD11BDAA9FC1A22L;

		private readonly int blocksize = 64;
		private readonly int rounds = 72;
		private readonly int words = 8;
		...
		/// </summary>
		private static readonly int[][] R8 = new int[][]
		{
			new int[] {46, 36, 19, 37},
			new int[] {33, 27, 14, 42},
			new int[] {17, 49, 36, 39},
		...
		public virtual void init(bool forEncryption, TweakableBlockCipherParameters @params)
		{
			// TODO: Remove some of the NPEs that can be avoided in the Params
			// classes
			if ((@params.getKey() == null) || (@params.getKey().getKey() == null) || (@params.getKey().getKey().Length != blocksize))
			{
		...
		private void fu()
		{
			...
			{
				JavaSystem.err.println(i + " : " + block[i].ToString("x"));
			}
			mix(0, 4);
