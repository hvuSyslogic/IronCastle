...
namespace org.bouncycastle.crypto.test.speedy
{
	...
	public class ThroughputTest
	{
		...
		private const int DATA_SIZE = 100 * 1024 * 1024;
	...
	//        testTF_256_1();
	//        testTF_256_2();
			JavaSystem.@out.println("Initialising test data.");
			byte[] input = new byte[DATA_SIZE];
			rand.nextBytes(input);

			JavaSystem.@out.println("Init complete.");
	...
	//        speedTestCipher(new BlowfishEngine(), input);
		...
		private static void runTestVector(string name, byte[] key, byte[] tweak, byte[] plaintext, byte[] expected, BlockCipher cipher)
		{
			...
			JavaSystem.@out.println("Plaintext  : " + StringHelper.NewString(Hex.encode(plaintext)));
			JavaSystem.@out.println("Expected   : " + StringHelper.NewString(Hex.encode(expected)));
			JavaSystem.@out.println("Ciphertext : " + StringHelper.NewString(Hex.encode(ciphertext)));
			JavaSystem.@out.println("  Encrypt  : " + Arrays.areEqual(expected, ciphertext));
			...
			JavaSystem.@out.println("Replain    : " + StringHelper.NewString(Hex.encode(replain)));
			JavaSystem.@out.println("  Decrypt  : " + Arrays.areEqual(plaintext, replain));
		}
		...
		{
			JavaSystem.@out.println("======");
			JavaSystem.@out.println("Testing " + cipher.getAlgorithmName() + " " + cipher.getBlockSize() * 8 + " " + mode);
			JavaSystem.@out.println("Beginning warmup run.");

			long warmup = testCipher(cipher, input);
			JavaSystem.@out.println("Warmup run 1 in " + (warmup / 1000000) + "ms");
			Thread.sleep(100);
			warmup = testCipher(cipher, input);
			JavaSystem.@out.println("Warmup run 2 in " + (warmup / 1000000) + "ms");
			...
			JavaSystem.@out.println("Beginning " + RUNS + " hot runs.");
			...
			for (int i = 0; i < RUNS; i++)
			{
				...
				total += runtimes[i];
				JavaSystem.@out.println("Run " + (i + 1) + ": " + runtimes[i] / 100000 + "ms");
			}
			long averageRuntime = total / RUNS;
			JavaSystem.@out.println(cipher.getAlgorithmName() + " Average run time: " + averageRuntime / 1000000 + "ms");
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final long mbPerSecond = (long)((double)DATA_SIZE / averageRuntime * 1000000000 / (1024 * 1024));
			long mbPerSecond = (long)((double)DATA_SIZE / averageRuntime * 1000000000 / (1024 * 1024));
			JavaSystem.@out.println(cipher.getAlgorithmName() + " Average speed:    " + mbPerSecond + " MB/s");
			JavaSystem.@out.println(cipher.getAlgorithmName() + " Average speed:    " + CLOCK_SPEED / (double)(mbPerSecond * (1024 * 1024)) + " c/b");
		}
	...
	//            JavaSystem.arraycopy(input, i, test, 0, test.length);
