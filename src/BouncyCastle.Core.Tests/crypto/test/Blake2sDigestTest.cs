namespace org.bouncycastle.crypto.test
{
	...
	public class Blake2sDigestTest : SimpleTest
	{
		...
		private static readonly string[][] keyedTestVectors = new string[][]
		{
			...
			new string[] {"00", "000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f", "40d15fee7c328830166ac3f918650f807e7e01e177258cdc0a39b11f598066f1"},
			new string[] {"0001", "000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f", "6bb71300644cd3991b26ccd4d274acd1adeab8b1d7914546c1198bbe9fc9d803"},
			new string[] {"000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f202122232425262728292a2b2c2d2e2f303132333435363738393a3b3c3d3e3f404142434445464748494a4b4c4d", "000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f", "172ffc67153d12e0ca76a8b6cd5d4731885b39ce0cac93a8972a18006c8b8baf"},
		...
		public virtual void testDigestWithKeyedTestVectors()
		{
			...
			for (int i = 0; i != keyedTestVectors.Length; i++)
			{
				...
				{
					fail("BLAKE2s mismatch on test vector ", keyedTestVector[2], StringHelper.NewString(Hex.encode(hash)));
				}
			}
		}

		public virtual void testDigestWithKeyedTestVectorsAndRandomUpdate()
		{
			Blake2sDigest digest = new Blake2sDigest(Hex.decode(keyedTestVectors[0][1]));
		...
		public virtual void testReset()
		{
			...
			{
				fail("BLAKE2s mismatch on test vector ", StringHelper.NewString(Hex.encode(hash)), StringHelper.NewString(Hex.encode(hash1)));
			}
		}

		// Self-test routine from https://tools.ietf.org/html/rfc7693#appendix-E
		private const string SELF_TEST_RESULT = "6A411F08CE25ADCDFB02ABA641451CEC53C598B24F4FC787FBDC88797F4C1DFE";
		private static readonly int[] SELF_TEST_DIGEST_LEN = new int[] {16, 20, 28, 32};
		private static readonly int[] SELF_TEST_INPUT_LEN = new int[] {0, 3, 64, 65, 255, 1024};

		private static byte[] selfTestSequence(int len, int seed)
		{
			int a = unchecked((int)0xDEAD4BAD) * seed;
			int b = 1;
			int t;
			...
			for (int i = 0; i < len; i++)
			{
				...
				a = b;
				b = t;
				@out[i] = unchecked((byte)((t >> 24) & 0xFF));
			}

			return @out;
		}

		public virtual void runSelfTest()
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				for (int j = 0; j < 6; j++)
				{
					...
					Blake2sDigest keyedDigest = new Blake2sDigest(key, outlen, null, null);
			...
			{
				fail("BLAKE2s mismatch on test vector ", SELF_TEST_RESULT, StringHelper.NewString(Hex.encode(hash)));
			}
		}
