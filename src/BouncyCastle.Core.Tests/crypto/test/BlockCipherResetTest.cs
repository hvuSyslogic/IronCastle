...
namespace org.bouncycastle.crypto.test
{
	...
	/// </summary>
	public class BlockCipherResetTest : SimpleTest
	{
		...
		private void testReset(string test, bool testCryptReset, BlockCipher cipher1, BlockCipher cipher2, CipherParameters @params)
		{
			cipher1.init(true, @params);

			byte[] plaintext = new byte[cipher1.getBlockSize()];
			byte[] ciphertext = new byte[(cipher1.getAlgorithmName().IndexOf("PGPCFBwithIV", StringComparison.Ordinal)) > -1 ? 2 * cipher1.getBlockSize() + 2 : cipher1.getBlockSize()];
		...
		private void checkReset(string test, bool testCryptReset, BlockCipher cipher, CipherParameters @params, bool encrypt, byte[] pretext, byte[] posttext)
		{
			// Do initial run
		...
		private static void crypt(BlockCipher cipher1, bool encrypt, byte[] plaintext, byte[] output)
		{
			cipher1.processBlock(plaintext, 0, output, 0);
			if ((cipher1.getAlgorithmName().IndexOf("PGPCFBwithIV", StringComparison.Ordinal) > -1) && !encrypt)
			{
