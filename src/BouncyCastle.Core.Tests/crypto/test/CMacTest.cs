namespace org.bouncycastle.crypto.test
{
	...
	using CMacWithIV = org.bouncycastle.crypto.macs.CMacWithIV;
	...
	public class CMacTest : SimpleTest
	{
		...
		public override void performTest()
		{
			...
			if (areEqual(@out, output_k256_m16))
			{
				fail("Failed - got " + StringHelper.NewString(Hex.encode(output_k256_m16)));
			}

			if (!areEqual(@out, Hex.decode("9347a60c64061b9ff2a92522ca8e08fc")))
			{
				fail("Failed - expected " + "9347a60c64061b9ff2a92522ca8e08fc" + " got " + StringHelper.NewString(Hex.encode(@out)));
			}
		...
		private void testCMac(BlockCipher cipher, byte[] keyBytes, byte[] input, byte[] expected)
		{
			...
			{
				fail("Failed - expected " + Strings.fromByteArray(Hex.encode(expected)) + " got " + StringHelper.NewString(Hex.encode(@out)));
			}
		}
