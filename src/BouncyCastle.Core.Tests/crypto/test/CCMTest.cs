namespace org.bouncycastle.crypto.test
{
	...
	/// </summary>
	public class CCMTest : SimpleTest
	{
		...
		public override void performTest()
		{
			...
			{
				JavaSystem.arraycopy(A4, 0, a4, i, A4.Length);
			}
			...
			JavaSystem.arraycopy(C2, 0, inBuf, 10, C2.Length);
			...
			AEADTestUtil.testOutputSizes(this, new CCMBlockCipher(new AESEngine()), new AEADParameters(new KeyParameter(K1), 32, N2));
			AEADTestUtil.testBufferSizeChecks(this, new CCMBlockCipher(new AESEngine()), new AEADParameters(new KeyParameter(K1), 32, N2));
		}
		...
		private void checkVectors(int count, CCMBlockCipher ccm, byte[] k, int macSize, byte[] n, byte[] a, byte[] p, byte[] t, byte[] c)
		{
		...
		private void checkVectors(int count, CCMBlockCipher ccm, string additionalDataType, byte[] k, int macSize, byte[] n, byte[] a, byte[] sa, byte[] p, byte[] t, byte[] c)
		{
			...
			JavaSystem.arraycopy(tmp, 0, dec, 0, len);

			if (!areEqual(p, dec))
			{
				fail("decrypted stream fails to match in test " + count + " with " + additionalDataType, StringHelper.NewString(Hex.encode(p)), StringHelper.NewString(Hex.encode(dec)));
			}
