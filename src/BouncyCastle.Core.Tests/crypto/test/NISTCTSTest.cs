namespace org.bouncycastle.crypto.test
{
	...
	/// CTS tester
	/// </summary>
	public class NISTCTSTest : SimpleTest
	{
		...
		private void testCTS(int id, int type, BlockCipher cipher, CipherParameters @params, byte[] input, byte[] output)
		{
			...
			{
				fail(id + " failed encryption expected " + StringHelper.NewString(Hex.encode(output)) + " got " + StringHelper.NewString(Hex.encode(@out)));
			}
			...
			{
				fail(id + " failed decryption expected " + StringHelper.NewString(Hex.encode(input)) + " got " + StringHelper.NewString(Hex.encode(@out)));
			}
		}
