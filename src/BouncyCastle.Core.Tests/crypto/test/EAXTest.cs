namespace org.bouncycastle.crypto.test
{
	...
	public class EAXTest : SimpleTest
	{
		...
		public override void performTest()
		{
			...
			AEADTestUtil.testOutputSizes(this, new EAXBlockCipher(new AESEngine()), new AEADParameters(new KeyParameter(K1), 32, N2));
			AEADTestUtil.testBufferSizeChecks(this, new EAXBlockCipher(new AESEngine()), new AEADParameters(new KeyParameter(K1), 32, N2));
		}
		...
		private void checkVectors(int count, string additionalDataType, byte[] k, int macSize, byte[] n, byte[] a, byte[] sa, byte[] p, byte[] t, byte[] c)
		{
		...
		private void runCheckVectors(int count, EAXBlockCipher encEax, EAXBlockCipher decEax, string additionalDataType, byte[] sa, byte[] p, byte[] t, byte[] c)
		{
		...
		private void ivParamTest(int count, AEADBlockCipher eax, byte[] k, byte[] n)
		{
		...
		{
			int DAT_LEN = (int)((uint)srng.nextInt() >> 22); // Note: JDK1.0 compatibility
