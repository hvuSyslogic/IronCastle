...
namespace org.bouncycastle.crypto.test
{
	...
	/// </summary>
	public class ECGOST3410Test : SimpleTest
	{
		...
		internal byte[] hashmessage = Hex.decode("3042453136414534424341374533364339313734453431443642453241453435");
		 ...
		 /// </summary>
		internal BigInteger r = new BigInteger("29700980915817952874371204983938256990422752107994319651632687982059210933395");
		internal BigInteger s = new BigInteger("574973400270084654178925310019147038455227042649098563933718999175515839552");

		internal byte[] kData = new BigInteger("53854137677348463731403841147996619241504003434302020712960838528893196233395").toByteArray();

		internal SecureRandom k;
		...
		private void ecGOST3410_TEST()
		{
			...
			ECCurve.Fp curve = new ECCurve.Fp(mod_p, new BigInteger("7"), new BigInteger("43308876546767276905765904595650931995942111794451039583252968842033849580414"), mod_q, ECConstants_Fields.ONE);

			ECDomainParameters @params = new ECDomainParameters(curve, curve.createPoint(new BigInteger("2"), new BigInteger("4018974056539037503335449422937059775635739389905545080690979365213431566280")), mod_q);

			ECPrivateKeyParameters priKey = new ECPrivateKeyParameters(new BigInteger("55441196065363246126355624130324183196576709222340016572108097750006097525544"), @params);
			...
			ECPublicKeyParameters pubKey = new ECPublicKeyParameters(curve.createPoint(new BigInteger("57520216126176808443631405023338071176630104906313632182896741342206604859403"), new BigInteger("17614944419213781543809391949654080031942662045363639260709847859438286763994")), @params);
		...
		/// </summary>
		public virtual void ecGOST3410_AParam()
		{
			SecureRandom random = new SecureRandom();
			...
			ECDomainParameters @params = new ECDomainParameters(curve, curve.createPoint(new BigInteger("1"), new BigInteger("64033881142927202683649881450433473985931760268884941288852745803908878638612")), mod_q);
		...
		private void ecGOST3410_BParam()
		{
			...
			ECCurve.Fp curve = new ECCurve.Fp(mod_p, new BigInteger("57896044618658097711785492504343953926634992332820282019728792003956564823190"), new BigInteger("28091019353058090096996979000309560759124368558014865957655842872397301267595"), mod_q, ECConstants_Fields.ONE);

			ECDomainParameters @params = new ECDomainParameters(curve, curve.createPoint(new BigInteger("1"), new BigInteger("28792665814854611296992347458380284135028636778229113005756334730996303888124")), mod_q);
		...
		private void ecGOST3410_CParam()
		{
			...
			ECCurve.Fp curve = new ECCurve.Fp(mod_p, new BigInteger("70390085352083305199547718019018437841079516630045180471284346843705633502616"), new BigInteger("32858"), mod_q, ECConstants_Fields.ONE);

			ECDomainParameters @params = new ECDomainParameters(curve, curve.createPoint(new BigInteger("0"), new BigInteger("29818893917731240733471273240314769927240550812383695689146495261604565990247")), mod_q);
