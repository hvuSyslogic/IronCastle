...
namespace org.bouncycastle.jcajce.provider.asymmetric.x509
{
	...
	public class X509CRLObject : X509CRL
	{
		...
		public X509CRLObject(JcaJceHelper bcHelper, CertificateList c)
		{
		...
		private void doVerify(PublicKey key, Signature sig)
		{
			...
			if (sigAlgParams != null)
			{
				...
				{
					throw new SignatureException("cannot decode signature parameters: " + e.Message);
				}
			}
				...
				while (certs.hasMoreElements())
				{
					...
					if (entry.getUserCertificate().getValue().Equals(serial))
					{
						...
						else
						{
							...
							{
								throw new IllegalArgumentException("Cannot process certificate: " + e.Message);
							}
						}
