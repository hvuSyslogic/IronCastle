namespace org.bouncycastle.jce.provider
{
	using DataLengthException = org.bouncycastle.crypto.DataLengthException;
	using DerivationFunction = org.bouncycastle.crypto.DerivationFunction;
	using DerivationParameters = org.bouncycastle.crypto.DerivationParameters;
	using Digest = org.bouncycastle.crypto.Digest;
	using OutputLengthException = org.bouncycastle.crypto.OutputLengthException;
	using KDFParameters = org.bouncycastle.crypto.@params.KDFParameters;

	/// <summary>
	/// Generator for PBE derived keys and ivs as defined by IEEE P1363a
	/// <br>
	/// This implementation is based on draft 9 of IEEE P1363a. <b>Note:</b>
	/// as this is still a draft the output of this generator may change, don't
	/// use it for anything that might be subject to long term storage.
	/// </summary>
	public class BrokenKDF2BytesGenerator : DerivationFunction
	{
		private Digest digest;
		private byte[] shared;
		private byte[] iv;

		/// <summary>
		/// Construct a KDF2 Parameters generator. Generates key material
		/// according to IEEE P1363a - if you want orthodox results you should
		/// use a digest specified in the standard.
		/// <para>
		/// <b>Note:</b> IEEE P1363a standard is still a draft standard, if the standard
		/// changes this function, the output of this function will change as well.
		/// Don't use this routine for anything subject to long term storage.
		/// 
		/// </para>
		/// </summary>
		/// <param name="digest"> the digest to be used as the source of derived keys. </param>
		public BrokenKDF2BytesGenerator(Digest digest)
		{
			this.digest = digest;
		}

		public virtual void init(DerivationParameters param)
		{
			if (!(param is KDFParameters))
			{
				throw new IllegalArgumentException("KDF parameters required for generator");
			}

			KDFParameters p = (KDFParameters)param;

			shared = p.getSharedSecret();
			iv = p.getIV();
		}

		/// <summary>
		/// return the underlying digest.
		/// </summary>
		public virtual Digest getDigest()
		{
			return digest;
		}

		/// <summary>
		/// fill len bytes of the output buffer with bytes generated from
		/// the derivation function.
		/// </summary>
		/// <exception cref="IllegalArgumentException"> if the size of the request will cause an overflow. </exception>
		/// <exception cref="DataLengthException"> if the out buffer is too small. </exception>
		public virtual int generateBytes(byte[] @out, int outOff, int len)
		{
			if ((@out.Length - len) < outOff)
			{
				throw new OutputLengthException("output buffer too small");
			}

			long oBits = len * 8L;

			//
			// this is at odds with the standard implementation, the
			// maximum value should be hBits * (2^32 - 1) where hBits
			// is the digest output size in bits. We can't have an
			// array with a long index at the moment...
			//
			if (oBits > (digest.getDigestSize() * 8L * (1L << 32 - 1)))
			{
				throw new IllegalArgumentException("Output length too large");
			}

			int cThreshold = (int)(oBits / digest.getDigestSize());

			byte[] dig = null;

			dig = new byte[digest.getDigestSize()];

			for (int counter = 1; counter <= cThreshold; counter++)
			{
				digest.update(shared, 0, shared.Length);

				digest.update(unchecked((byte)(counter & 0xff)));
				digest.update(unchecked((byte)((counter >> 8) & 0xff)));
				digest.update(unchecked((byte)((counter >> 16) & 0xff)));
				digest.update(unchecked((byte)((counter >> 24) & 0xff)));

				digest.update(iv, 0, iv.Length);

				digest.doFinal(dig, 0);

				if ((len - outOff) > dig.Length)
				{
					JavaSystem.arraycopy(dig, 0, @out, outOff, dig.Length);
					outOff += dig.Length;
				}
				else
				{
					JavaSystem.arraycopy(dig, 0, @out, outOff, len - outOff);
				}
			}

			digest.reset();

			return len;
		}
	}

}