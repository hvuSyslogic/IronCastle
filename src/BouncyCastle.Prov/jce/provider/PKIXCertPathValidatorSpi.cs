namespace org.bouncycastle.jce.provider
{
	...
	/// 3280.
	/// </summary>
	public class PKIXCertPathValidatorSpi : CertPathValidatorSpi
	{
		...
		public virtual CertPathValidatorResult engineValidate(CertPath certPath, CertPathParameters @params)
		{
			...
			else if (@params is PKIXExtendedParameters)
			{
				paramsPKIX = (PKIXExtendedParameters)@params;
			}
			else
			{
				throw new InvalidAlgorithmParameterException("Parameters must be a " + typeof(PKIXParameters).getName() + " instance.");
			}
			...
			{
				throw new CertPathValidatorException(e.Message, e.getUnderlyingException(), certPath, certs.size() - 1);
			}
			...
			for (index = certs.size() - 1; index >= 0; index--)
			{
				...
				bool verificationAlreadyPerformed = (index == certs.size() - 1);
				...
				{
					throw new CertPathValidatorException(e.Message, e.getUnderlyingException(), certPath, index);
				}
