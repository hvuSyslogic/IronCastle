...
namespace org.bouncycastle.jce.provider
{
	...
	public class PKIXNameConstraintValidator
	{
		...
		public virtual void checkPermittedDN(ASN1Sequence dns)
		{
			checkPermittedDN(permittedSubtreesDN, dns);
		}

		public virtual void checkExcludedDN(ASN1Sequence dns)
		{
		...
		///          if the IP is not permitted. </exception>
		...
		/// <param name="ip">       The IP address. </param>
		/// <exception cref="PKIXNameConstraintValidatorException">
		///          if the IP is excluded. </exception>
		...
		private bool emailIsConstrained(string email, string constraint)
		{
			...
			if (constraint.IndexOf('@') != -1)
			{
				...
				if (sub.Equals(constraint.Substring(1), StringComparison.OrdinalIgnoreCase))
				{
					return true;
				}
			}
		...
		public virtual Set unionDNS(Set excluded, string dns)
		{
			if (excluded.isEmpty())
			{
				if (string.ReferenceEquals(dns, null))
		...
		/// <exception cref="PKIXNameConstraintValidatorException">
		///          If the <code>name</code> </exception>
		...
		private string stringifyIP(byte[] ip)
		{
			...
			{
				temp += Convert.ToString(ip[i] & 0x00FF) + ".";
			}
