...
namespace org.bouncycastle.pqc.jcajce.provider.xmss
{
	...
	public class DigestUtil
	{
		...
		{
			if (treeDigest.Equals(NISTObjectIdentifiers_Fields.id_sha256))
			{
				return XMSSParameterSpec.SHA256;
			}
			if (treeDigest.Equals(NISTObjectIdentifiers_Fields.id_sha512))
			{
				return XMSSParameterSpec.SHA512;
			}
			if (treeDigest.Equals(NISTObjectIdentifiers_Fields.id_shake128))
			{
				return XMSSParameterSpec.SHAKE128;
			}
			if (treeDigest.Equals(NISTObjectIdentifiers_Fields.id_shake256))
			{
