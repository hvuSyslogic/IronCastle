...
namespace org.bouncycastle.pqc.jcajce.provider.xmss
{
	...
	public class XMSSKeyFactorySpi : KeyFactorySpi, AsymmetricKeyInfoConverter
	{
		...
		public KeySpec engineGetKeySpec(Key key, Class keySpec)
		{
			...
			else if (key is BCXMSSPublicKey)
			{
		...
		{
			if (key is BCXMSSPrivateKey || key is BCXMSSPublicKey)
			{
				return key;
			}
