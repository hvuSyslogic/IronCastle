...
namespace org.bouncycastle.pqc.jcajce.provider.qtesla
{
	...
	public class QTESLAKeyFactorySpi : KeyFactorySpi, AsymmetricKeyInfoConverter
	{
		...
		public KeySpec engineGetKeySpec(Key key, Class keySpec)
		{
			...
			else if (key is BCqTESLAPublicKey)
			{
		...
		{
			if (key is BCqTESLAPrivateKey || key is BCqTESLAPublicKey)
			{
				return key;
			}
