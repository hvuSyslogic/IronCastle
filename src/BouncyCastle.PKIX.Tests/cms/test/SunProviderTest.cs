...
namespace org.bouncycastle.cms.test
{
	...
	public class SunProviderTest : TestCase
	{
		...
		private void testKeyTrans(string algorithm)
		{
			...
			CMSEnvelopedData ed = edGen.generate(new CMSProcessableByteArray(data), (new JceCMSContentEncryptorBuilder(new ASN1ObjectIdentifier(algorithm))).setProvider("SunJCE").build());
		...
		{
			return new TestSuite(typeof(SunProviderTest));
		}
	}
