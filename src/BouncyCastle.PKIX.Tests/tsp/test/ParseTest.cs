...
namespace org.bouncycastle.tsp.test
{
	...
	{
		private byte[] sha1Request = Base64.decode("MDACAQEwITAJBgUrDgMCGgUABBT5UbEBmJssO3RxcQtOePxNvfoMpgIIC+Gv" + "YW2mtZQ=");


		private byte[] sha1noNonse = Base64.decode("MCYCAQEwITAJBgUrDgMCGgUABBT5UbEBmJssO3RxcQtOePxNvfoMpg==");

		private byte[] md5Request = Base64.decode("MDoCAQEwIDAMBggqhkiG9w0CBQUABBDIl9FBCvjyx0+6EbHbUR6eBgkrBgEE" + "AakHBQECCDQluayIxIzn");

		private byte[] ripemd160Request = Base64.decode("MD8CAQEwITAJBgUrJAMCAQUABBSq03a/mk50Yd9lMF+BSqOp/RHGQQYJKwYB" + "BAGpBwUBAgkA4SZs9NfqISMBAf8=");
		...
		private void requestParse(byte[] request, ASN1ObjectIdentifier algorithm)
		{
			TimeStampRequest req = new TimeStampRequest(request);

			if (!req.getMessageImprintAlgOID().Equals(algorithm))
			{
				fail("failed to get expected algorithm - got " + req.getMessageImprintAlgOID() + " not " + algorithm);
			}
			...
			try
			{
				req.validate(TSPAlgorithms_Fields.ALLOWED, null, null);
			}
			catch (Exception)
			{
		...
		private void responseParse(byte[] request, byte[] response, ASN1ObjectIdentifier algorithm)
		{
			TimeStampRequest req = new TimeStampRequest(request);
		...
		private void unacceptableResponseParse(byte[] response)
		{
			TimeStampResponse resp = new TimeStampResponse(response);
		...
		private void generalizedTimeParse(byte[] response)
		{
			TimeStampResponse resp = new TimeStampResponse(response);
		...
		{
			requestParse(sha1Request, TSPAlgorithms_Fields.SHA1);

			requestParse(sha1noNonse, TSPAlgorithms_Fields.SHA1);

			requestParse(md5Request, TSPAlgorithms_Fields.MD5);

			requestParse(ripemd160Request, TSPAlgorithms_Fields.RIPEMD160);

			responseParse(sha1Request, sha1Response, TSPAlgorithms_Fields.SHA1);

			responseParse(sha1noNonse, sha1noNonseResponse, TSPAlgorithms_Fields.SHA1);

			responseParse(md5Request, md5Response, TSPAlgorithms_Fields.MD5);
		...
		private void v2SigningResponseParse(byte[] encoded)
		{
		...
		public virtual void parse(byte[] encoded, bool tokenPresent)
		{
			TimeStampResponse response = new TimeStampResponse(encoded);
