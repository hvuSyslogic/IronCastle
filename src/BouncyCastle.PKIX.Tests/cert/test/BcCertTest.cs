...
namespace org.bouncycastle.cert.test
{
	...
	using BcX509v1CertificateBuilder = org.bouncycastle.cert.bc.BcX509v1CertificateBuilder;
	using BcX509v3CertificateBuilder = org.bouncycastle.cert.bc.BcX509v3CertificateBuilder;
	...
	using BcDSAContentSignerBuilder = org.bouncycastle.@operator.bc.BcDSAContentSignerBuilder;
	using BcDSAContentVerifierProviderBuilder = org.bouncycastle.@operator.bc.BcDSAContentVerifierProviderBuilder;
	using BcECContentSignerBuilder = org.bouncycastle.@operator.bc.BcECContentSignerBuilder;
	using BcECContentVerifierProviderBuilder = org.bouncycastle.@operator.bc.BcECContentVerifierProviderBuilder;
	...
	public class BcCertTest : TestCase
	{
		...
		public virtual void checkCertificate(int id, byte[] bytes)
		{
			try
			{
		 ...
		 */
		public virtual void checkKeyUsage(int id, byte[] bytes)
		{
		...
		public virtual void checkSelfSignedCertificate(int id, byte[] bytes)
		{
		...
		public virtual void checkCreation1()
		{
			...
			// a sample key pair.
			//
			AsymmetricKeyParameter pubKey = new RSAKeyParameters(false, new BigInteger("b4a7e46170574f16a97082b22be58b6a2a629798419be12872a4bdba626cfae9900f76abfb12139dce5de56564fab2b6543165a040c606887420e33d91ed7ed7", 16), new BigInteger("11", 16));
			...
			builder.addRDN(PKCSObjectIdentifiers_Fields.pkcs_9_at_emailAddress, "feedback-crypto@bouncycastle.org");
			...
			X509v3CertificateBuilder certGen = new BcX509v3CertificateBuilder(builder.build(), BigInteger.valueOf(1), new DateTime(System.currentTimeMillis() - 50000), new DateTime(System.currentTimeMillis() + 50000),builder.build(), pubKey);

			X509CertificateHolder certH = certGen.build(sigGen);

			assertTrue(certH.isValidOn(DateTime.Now));
			...
			//
			sigGen = (new BcRSAContentSignerBuilder(sigAlgFinder.find("MD5WithRSA"), digAlgFinder.find(sigAlgFinder.find("MD5withRSA")))).build(privKey);
			X509v1CertificateBuilder certGen1 = new BcX509v1CertificateBuilder(builder.build(), BigInteger.valueOf(1), new DateTime(System.currentTimeMillis() - 50000),new DateTime(System.currentTimeMillis() + 50000),builder.build(),pubKey);
	...
	//
			...
			certGen = new X509v3CertificateBuilder(builder.build(), BigInteger.valueOf(1), new DateTime(System.currentTimeMillis() - 50000), new DateTime(System.currentTimeMillis() + 50000), builder.build(), pubInfo);
		...
		/// </summary>
		public virtual void checkCreation2()
		{
			//
			// set up the keys
			//
			...
			assertTrue(cert.isValidOn(DateTime.Now));
		...
		/// </summary>
		 public virtual void checkCreation3()
		 {
			 X9ECParameters x9 = ECNamedCurveTable.getByName("prime239v1");
			 ECCurve curve = x9.getCurve();
			 ECDomainParameters @params = new ECDomainParameters(curve, x9.getG(), x9.getN(), x9.getH());

			 ECPrivateKeyParameters privKey = new ECPrivateKeyParameters(new BigInteger("876300101507107567501066130761671078357010671067781776716671676178726717"), @params);

			 ECPublicKeyParameters pubKey = new ECPublicKeyParameters(curve.decodePoint(Hex.decode("025b6dc53bc61a2548ffb0f671472de6c9521a9d2d2534e65abfcbd5fe0c70")), @params);
			 ...
			 try
			 {
				 ContentSigner sigGen = (new BcECContentSignerBuilder(new AlgorithmIdentifier(X9ObjectIdentifiers_Fields.ecdsa_with_SHA1), new AlgorithmIdentifier(OIWObjectIdentifiers_Fields.idSHA1))).build(privKey);
				 BcX509v3CertificateBuilder certGen = new BcX509v3CertificateBuilder(builder.build(), BigInteger.valueOf(1), new DateTime(System.currentTimeMillis() - 50000), new DateTime(System.currentTimeMillis() + 50000), builder.build(), pubKey);

				 X509CertificateHolder cert = certGen.build(sigGen);

				 if (!cert.isValidOn(DateTime.Now))
				 {
		...
		public virtual void checkCRLCreation1()
		{
		...
		public virtual void checkCRLCreation2()
		{
			AsymmetricCipherKeyPairGenerator kpg = new RSAKeyPairGenerator();
			RSAKeyGenerationParameters genParam = new RSAKeyGenerationParameters(BigInteger.valueOf(0x1001), new SecureRandom(), 1024, 25);
		...
		public virtual void checkCRLCreation3()
		{
			AsymmetricCipherKeyPairGenerator kpg = new RSAKeyPairGenerator();
			RSAKeyGenerationParameters genParam = new RSAKeyGenerationParameters(BigInteger.valueOf(0x1001), new SecureRandom(), 1024, 25);
		...
		public virtual void checkCreation5()
		{
			//
			// a sample key pair.
			//
			...
			certGen = (new BcX509v3CertificateBuilder(builder.build(),BigInteger.valueOf(1),new DateTime(System.currentTimeMillis() - 50000),new DateTime(System.currentTimeMillis() + 50000),builder.build(),pubKey)).copyAndAddExtension(new ASN1ObjectIdentifier("2.5.29.15"), true, baseCert).copyAndAddExtension(extOid, false, baseCert);
	...
	//        }
		...
		public virtual void testForgedSignature()
		{
		...
		private void pemTest()
		{
			...
			CRL crl = cf.generateCRL(new ByteArrayInputStream(PEMData.CRL_1.GetBytes(Encoding.ASCII)));
			if (crl == null)
			{
				fail("PEM crl not read");
			}
			Collection col = cf.generateCertificates(new ByteArrayInputStream(PEMData.CERTIFICATE_2.GetBytes(Encoding.ASCII)));
			if (col.size() != 1 || !col.contains(cert))
			{
				fail("PEM cert collection not right");
			}
			col = cf.generateCRLs(new ByteArrayInputStream(PEMData.CRL_2.GetBytes(Encoding.ASCII)));
		...
		{
			return (X509Certificate)cf.generateCertificate(new ByteArrayInputStream(pemData.GetBytes(Encoding.ASCII)));
		}
		...
		public virtual void testNullDerNullCert()
		{
			...
			ContentSigner sigGen = (new BcRSAContentSignerBuilder(sigAlgId, digAlgId)).build(privKey);
			BcX509v3CertificateBuilder certGen = new BcX509v3CertificateBuilder(new X500Name("CN=Test"),BigInteger.valueOf(1),new DateTime(System.currentTimeMillis() - 50000),new DateTime(System.currentTimeMillis() + 50000),new X500Name("CN=Test"),pubKey);
			X509CertificateHolder cert = certGen.build(sigGen);
		...
		public virtual void testCertificates()
		{
