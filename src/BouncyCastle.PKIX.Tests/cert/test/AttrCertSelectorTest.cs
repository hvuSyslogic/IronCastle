...
namespace org.bouncycastle.cert.test
{
	...
	public class AttrCertSelectorTest : SimpleTest
	{
		...
		private X509AttributeCertificateHolder createAttrCert()
		{
			...
			X509Certificate iCert = (X509Certificate) fact.generateCertificate(new ByteArrayInputStream(holderCert));
			...
			X509v2AttributeCertificateBuilder gen = new X509v2AttributeCertificateBuilder(new AttributeCertificateHolder(iCertHolder.getSubject()), new AttributeCertificateIssuer(new X500Name("cn=test")), BigInteger.valueOf(1), new DateTime(System.currentTimeMillis() - 50000), new DateTime(System.currentTimeMillis() + 50000));
		...
		public virtual void testSelector()
		{
			...
			CertificateFactory fact = CertificateFactory.getInstance("X.509", "BC");
			X509CertificateHolder iCert = new JcaX509CertificateHolder((X509Certificate) fact.generateCertificate(new ByteArrayInputStream(holderCert)));
