...
namespace org.bouncycastle.cert.ocsp.test
{
	...
	using JcaBasicOCSPRespBuilder = org.bouncycastle.cert.ocsp.jcajce.JcaBasicOCSPRespBuilder;
	...
	public class OCSPTest : SimpleTest
	{
		...
		private void testECDSA()
		{
			...
			extGen.addExtension(OCSPObjectIdentifiers_Fields.id_pkix_ocsp_nonce, false, new DEROctetString(sampleNonce));
			...
			Extension extValue = req.getExtension(OCSPObjectIdentifiers_Fields.id_pkix_ocsp_nonce);
			...
			respGen.addResponse(id, CertificateStatus_Fields.GOOD);

			BasicOCSPResp resp = respGen.build((new JcaContentSignerBuilder("SHA1withECDSA")).setProvider(BC).build(signKP.getPrivate()), chain, DateTime.Now);
		}
		...
		private void testRSA()
		{
			...
			Extension ext = req.getExtension(OCSPObjectIdentifiers_Fields.id_pkix_ocsp_nonce);
			...
			BasicOCSPResp resp = respGen.build((new JcaContentSignerBuilder("SHA1withRSA")).setProvider(BC).build(signKP.getPrivate()), chain, DateTime.Now);
		...
		public virtual void testInvalidResp()
		{
			try
			{
			...
			{
				if (e.InnerException is ASN1Exception)
				{
					Exception c = ((ASN1Exception)e.InnerException).InnerException;

					if (!c.Message.Equals("ENUMERATED has zero length"))
					{
						fail("parsing failed, but for wrong reason: " + c.Message);
					}
				}
				else
				{
					fail("parsing failed, but for wrong reason: " + e.Message);
				}
			}
		...
		public override void performTest()
		{
			...
			{
				 if (!e.Message.Equals("malformed response: no response data found"))
				 {
			...
			{
				 if (!e.Message.Equals("malformed request: no request data found"))
				 {
					 fail("wrong exception");
				 }
			}
