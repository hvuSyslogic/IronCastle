namespace org.bouncycastle.cert.path.test
{
	...
	using JcaX509ContentVerifierProviderBuilder = org.bouncycastle.cert.jcajce.JcaX509ContentVerifierProviderBuilder;
	using BasicConstraintsValidation = org.bouncycastle.cert.path.validations.BasicConstraintsValidation;
	using CRLValidation = org.bouncycastle.cert.path.validations.CRLValidation;
	using KeyUsageValidation = org.bouncycastle.cert.path.validations.KeyUsageValidation;
	using ParentCertIssuedValidation = org.bouncycastle.cert.path.validations.ParentCertIssuedValidation;
	...
	public class CertPathValidationTest : SimpleTest
	{
	...
	//    }
		...
		public override void performTest()
		{
			...
			result = path.evaluate(new CertPathValidation[]
			{
				new ParentCertIssuedValidation(verifier),
				new BasicConstraintsValidation(),
				new KeyUsageValidation()
			...
			result = path.validate(new CertPathValidation[]
			{
				new ParentCertIssuedValidation(verifier),
				new BasicConstraintsValidation(),
				new KeyUsageValidation(),
				new CRLValidation(rootCert.getSubject(), crls)
			});
			...
			if (result.isValid() || result.getUnhandledCriticalExtensionOIDs().size() != 1 || !result.getUnhandledCriticalExtensionOIDs().contains(Extension.basicConstraints))
			{
			...
			if (result.isValid() || result.getUnhandledCriticalExtensionOIDs().size() != 2 || !result.getUnhandledCriticalExtensionOIDs().contains(Extension.basicConstraints) || !result.getUnhandledCriticalExtensionOIDs().contains(Extension.keyUsage))
			{
