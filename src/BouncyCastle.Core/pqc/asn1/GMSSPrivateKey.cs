...
namespace org.bouncycastle.pqc.asn1
{
	...
	public class GMSSPrivateKey : ASN1Object
	{
		...
		/// <param name="currentSeeds">      seed for the generation of private OTS keys for the
		///                          current subtrees (TREE) </param>
		/// <param name="nextNextSeeds">     seed for the generation of private OTS keys for the
		///                          subtrees after next (TREE++) </param>
		/// <param name="currentAuthPaths">  array of current authentication paths (AUTHPATH) </param>
		/// <param name="nextAuthPaths">     array of next authentication paths (AUTHPATH+) </param>
		...
		/// <param name="algorithms">        An array of algorithm identifiers, containing the hash function details </param>
		...
		private static int checkBigIntegerInIntRange(ASN1Encodable a)
		{
			...
			if ((b.compareTo(BigInteger.valueOf(int.MaxValue)) > 0) || (b.compareTo(BigInteger.valueOf(int.MinValue)) < 0))
			{
