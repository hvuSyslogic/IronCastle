...
namespace org.bouncycastle.pqc.crypto.xmss
{
	...
	public class XMSSSigner : StateAwareMessageSigner
	{
		...
		public virtual byte[] generateSignature(byte[] message)
		{
			...
			byte[] concatenated = Arrays.concatenate(random, privateKey.getRoot(), XMSSUtil.toBytesBigEndian(index, @params.getDigestSize()));
		...
		public virtual bool verifySignature(byte[] message, byte[] signature)
		{
			...
			byte[] concatenated = Arrays.concatenate(sig.getRandom(), publicKey.getRoot(), XMSSUtil.toBytesBigEndian(index, @params.getDigestSize()));
			byte[] messageDigest = khf.HMsg(concatenated, message);
