...
namespace org.bouncycastle.pqc.crypto.xmss
{
	public class XMSSVerifierUtil
	{
		...
		/// </summary>
		/// <param name="messageDigest"> Message digest. </param>
		/// <param name="signature">     XMSS signature. </param>
		/// <returns> Root node calculated from signature. </returns>
		internal static XMSSNode getRootNodeFromSignature(WOTSPlus wotsPlus, int height, byte[] messageDigest, XMSSReducedSignature signature, OTSHashAddress otsHashAddress, int indexLeaf)
		{
			 ...
			 */
			WOTSPlusPublicKeyParameters wotsPlusPK = wotsPlus.getPublicKeyFromSignature(messageDigest, signature.getWOTSPlusSignature(), otsHashAddress);
			XMSSNode[] node = new XMSSNode[2];
			node[0] = XMSSNodeUtil.lTree(wotsPlus, wotsPlusPK, lTreeAddress);

			for (int k = 0; k < height; k++)
			{
				hashTreeAddress = (HashTreeAddress)(new HashTreeAddress.Builder()).withLayerAddress(hashTreeAddress.getLayerAddress()).withTreeAddress(hashTreeAddress.getTreeAddress()).withTreeHeight(k).withTreeIndex(hashTreeAddress.getTreeIndex()).withKeyAndMask(hashTreeAddress.getKeyAndMask()).build();
				if (Math.Floor(indexLeaf / (float)(1 << k)) % 2 == 0)
				{
