...
namespace org.bouncycastle.pqc.crypto.xmss
{
	...
	{
		private const long serialVersionUID = -3464451825208522308L;

		private readonly Map<int, BDS> bdsState = new TreeMap<int, BDS>();
		...
		private void updateState(XMSSMTParameters @params, long globalIndex, byte[] publicSeed, byte[] secretKeySeed)
		{
			...
			OTSHashAddress otsHashAddress = (OTSHashAddress)(new OTSHashAddress.Builder()).withTreeAddress(indexTree).withOTSAddress(indexLeaf).build();
			...
			for (int layer = 1; layer < @params.getLayers(); layer++)
			{
				...
				indexTree = XMSSUtil.getTreeIndex(indexTree, xmssHeight);
					/* adjust addresses */
				otsHashAddress = (OTSHashAddress)(new OTSHashAddress.Builder()).withLayerAddress(layer).withTreeAddress(indexTree).withOTSAddress(indexLeaf).build();
		...
		public virtual BDS get(int index)
		{
			return bdsState.get(Integers.valueOf(index));
		}

		public virtual BDS update(int index, byte[] publicSeed, byte[] secretKeySeed, OTSHashAddress otsHashAddress)
		{
		...
		public virtual void put(int index, BDS bds)
		{
			bdsState.put(Integers.valueOf(index), bds);
		}

		public virtual BDSStateMap withWOTSDigest(ASN1ObjectIdentifier digestName)
		{
