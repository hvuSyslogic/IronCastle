...
namespace org.bouncycastle.pqc.crypto.mceliece
{
	...
	/// </summary>
	public class McElieceKobaraImaiCipher : MessageEncryptor
	{
		...
		/// </summary>
		public static readonly byte[] PUBLIC_CONSTANT = "a predetermined public constant" .getBytes();
		...
		public virtual byte[] messageEncrypt(byte[] input)
		{
			...
			JavaSystem.arraycopy(input, 0, mConst, 0, input.Length);
			JavaSystem.arraycopy(PUBLIC_CONSTANT, 0, mConst, mLen, PUBLIC_CONSTANT.Length);
			...
			{
				c6 = new byte[c6Len];
				JavaSystem.arraycopy(c2c1, 0, c6, 0, c6Len);
			}

			byte[] c5 = new byte[c5Len];
			JavaSystem.arraycopy(c2c1, c6Len, c5, 0, c5Len);

			byte[] c4 = new byte[c4Len];
			JavaSystem.arraycopy(c2c1, c6Len + c5Len, c4, 0, c4Len);
