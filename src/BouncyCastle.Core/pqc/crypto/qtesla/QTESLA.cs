...
//ORIGINAL LINE: private static void hashFunction(byte[] output, int outputOffset, int[] V, final byte[] message, int messageOffset, int n, int d, int q)
		private static void hashFunction(byte[] output, int outputOffset, int[] V, byte[] message, int messageOffset, int n, int d, int q)
		{
			...
			JavaSystem.arraycopy(message, messageOffset, T, n, Polynomial.MESSAGE);
...
//ORIGINAL LINE: private static void hashFunction(byte[] output, int outputOffset, long[] V, final byte[] message, int messageOffset, int n, int k, int d, int q)
		private static void hashFunction(byte[] output, int outputOffset, long[] V, byte[] message, int messageOffset, int n, int k, int d, int q)
		{
			...
			JavaSystem.arraycopy(message, messageOffset, T, n * k, Polynomial.MESSAGE);
		...
		/// <param name="u">        Bound in Checking Secret Polynomial
		/// </param>
		/// <returns> false    Valid / Accepted
		...
		/// <param name="rejection">    Bound in Checking Error Polynomial
		/// </param>
		/// <returns> false        Valid / Accepted
		/// 				true		Invalid / Rejected
		/// ******************************************************************************** </returns>
		...
		private static bool testV(long[] V, int vOffset, int n, int d, int q, int rejection)
		{
			...
			for (int i = 0; i < n; i++)
			{
				...
				test1 = (int)((uint)(~(absolute(right) - (q / 2 - rejection))) >> 63);

				left = right;
				right = (int)((right + (1 << (d - 1)) - 1) >> d);
				right = left - (right << d);
				test2 = (int)((uint)(~(absolute(right) - ((1 << (d - 1)) - rejection))) >> 63);
		...
		/// <param name="secretBound">                            Bound in Checking Secret Polynomial
		/// </param>
		...
		/// <param name="publicKey">                            Contains Public Key </param>
		/// <param name="privateKey">                            Contains Private Key </param>
		/// <param name="secureRandom">                        Source of Randomness
		/// </param>
		/// <returns> 0                                    Successful Execution
		/// 
		/// *************************************************************************************************************************************************************** </returns>
		...
		public static int generateKeyPairIIISize(byte[] publicKey, byte[] privateKey, SecureRandom secureRandom)
		{
			...
			return generateKeyPair(publicKey, privateKey, secureRandom, Parameter.N_III_SIZE, Parameter.H_III_SIZE, Parameter.Q_III_SIZE, Parameter.Q_INVERSE_III_SIZE, Parameter.Q_LOGARITHM_III_SIZE, Parameter.GENERATOR_A_III_SIZE, Parameter.INVERSE_NUMBER_THEORETIC_TRANSFORM_III_SIZE, Parameter.XI_III_SIZE, PolynomialHeuristic.ZETA_III_SIZE, Parameter.KEY_GENERATOR_BOUND_E_III_SIZE, Parameter.KEY_GENERATOR_BOUND_S_III_SIZE);
		...
		public static int generateKeyPairIIISpeed(byte[] publicKey, byte[] privateKey, SecureRandom secureRandom)
		{
			...
			return generateKeyPair(publicKey, privateKey, secureRandom, Parameter.N_III_SPEED, Parameter.H_III_SPEED, Parameter.Q_III_SPEED, Parameter.Q_INVERSE_III_SPEED, Parameter.Q_LOGARITHM_III_SPEED, Parameter.GENERATOR_A_III_SPEED, Parameter.INVERSE_NUMBER_THEORETIC_TRANSFORM_III_SPEED, Parameter.XI_III_SPEED, PolynomialHeuristic.ZETA_III_SPEED, Parameter.KEY_GENERATOR_BOUND_E_III_SPEED, Parameter.KEY_GENERATOR_BOUND_S_III_SPEED);
...
//ORIGINAL LINE: private static int signing(byte[] signature, final byte[] message, int messageOffset, int messageLength, final byte[] privateKey, java.security.SecureRandom secureRandom, int n, int h, int q, long qInverse, int qLogarithm, int b, int bBit, int d, int u, int rejection, int generatorA, int inverseNumberTheoreticTransform, int barrettMultiplication, int barrettDivision, int[] zeta)
	...
	//        rng.randomByte(randomnessInput, Polynomial.RANDOM, Polynomial.RANDOM);
			 ...
			 JavaSystem.arraycopy(temporaryRandomnessInput, 0, randomnessInput, Polynomial.RANDOM, Polynomial.RANDOM);

			JavaSystem.arraycopy(seed, Polynomial.SEED, randomnessInput, 0, Polynomial.SEED);
...
//ORIGINAL LINE: private static int signing(byte[] signature, final byte[] message, int messageOffset, int messageLength, final byte[] privateKey, java.security.SecureRandom secureRandom, int n, int k, int h, int q, long qInverse, int qLogarithm, int b, int bBit, int d, int u, int rejection, int generatorA, int inverseNumberTheoreticTransform, int privateKeySize, int barrettMultiplication, int barrettDivision)
	...
	//        rng.randomByte(randomnessInput, Polynomial.RANDOM, Polynomial.RANDOM);
			secureRandom.nextBytes(temporaryRandomnessInput);
			JavaSystem.arraycopy(temporaryRandomnessInput, 0, randomnessInput, Polynomial.RANDOM, Polynomial.RANDOM);
			JavaSystem.arraycopy(privateKey, privateKeySize - Polynomial.SEED, randomnessInput, 0, Polynomial.SEED);
...
//ORIGINAL LINE: private static int verifying(byte[] message, final byte[] signature, int signatureOffset, int signatureLength, final byte[] publicKey, int n, int h, int q, long qInverse, int qLogarithm, int b, int d, int u, int r, int signatureSize, int generatorA, int inverseNumberTheoreticTransform, int barrettMultiplication, int barrettDivision, int[] zeta)
		...
		/// <param name="signatureLength">                        Length of the Given Signature Package </param>
		/// <param name="message">                                Original (Signed) Message </param>
		/// <param name="publicKey">                            Public Key
		/// </param>
		/// <returns> 0                                    Valid Signature
		/// 				< 0									Invalid Signature
		/// ****************************************************************************************************** </returns>
...
//ORIGINAL LINE: static int verifyingIIISize(byte[] message, final byte[] signature, int signatureOffset, int signatureLength, final byte[] publicKey)
		internal static int verifyingIIISize(byte[] message, byte[] signature, int signatureOffset, int signatureLength, byte[] publicKey)
		{
			...
			return verifying(message, signature, signatureOffset, signatureLength, publicKey, Parameter.N_III_SIZE, Parameter.H_III_SIZE, Parameter.Q_III_SIZE, Parameter.Q_INVERSE_III_SIZE, Parameter.Q_LOGARITHM_III_SIZE, Parameter.B_III_SIZE, Parameter.D_III_SIZE, Parameter.U_III_SIZE, Parameter.R_III_SIZE, Polynomial.SIGNATURE_III_SIZE, Parameter.GENERATOR_A_III_SIZE, Parameter.INVERSE_NUMBER_THEORETIC_TRANSFORM_III_SIZE, Parameter.BARRETT_MULTIPLICATION_III_SIZE, Parameter.BARRETT_DIVISION_III_SIZE, PolynomialHeuristic.ZETA_III_SIZE);
...
//ORIGINAL LINE: static int verifyingIIISpeed(byte[] message, final byte[] signature, int signatureOffset, int signatureLength, final byte[] publicKey)
		internal static int verifyingIIISpeed(byte[] message, byte[] signature, int signatureOffset, int signatureLength, byte[] publicKey)
		{
			...
			return verifying(message, signature, signatureOffset, signatureLength, publicKey, Parameter.N_III_SPEED, Parameter.H_III_SPEED, Parameter.Q_III_SPEED, Parameter.Q_INVERSE_III_SPEED, Parameter.Q_LOGARITHM_III_SPEED, Parameter.B_III_SPEED, Parameter.D_III_SPEED, Parameter.U_III_SPEED, Parameter.R_III_SPEED, Polynomial.SIGNATURE_III_SPEED, Parameter.GENERATOR_A_III_SPEED, Parameter.INVERSE_NUMBER_THEORETIC_TRANSFORM_III_SPEED, Parameter.BARRETT_MULTIPLICATION_III_SPEED, Parameter.BARRETT_DIVISION_III_SPEED, PolynomialHeuristic.ZETA_III_SPEED);
