namespace org.bouncycastle.pqc.crypto.sphincs
{
	public class Horst
	{
		internal const int HORST_LOGT = 16;
		internal static readonly int HORST_T = (1 << HORST_LOGT);
		internal const int HORST_K = 32;
		internal const int HORST_SKBYTES = 32;
		internal static readonly int HORST_SIGBYTES = (64 * SPHINCS256Config.HASH_BYTES + (((HORST_LOGT - 6) * SPHINCS256Config.HASH_BYTES) + HORST_SKBYTES) * HORST_K);

		internal static readonly int N_MASKS = (2 * (Horst.HORST_LOGT)); // has to be the max of  (2*(SUBTREE_HEIGHT+WOTS_LOGL)) and (WOTS_W-1) and 2*HORST_LOGT

		internal static void expand_seed(byte[] outseeds, byte[] inseed)
		{
			Seed.prg(outseeds, 0, HORST_T * HORST_SKBYTES, inseed, 0);
		}

		internal static int horst_sign(HashFunctions hs, byte[] sig, int sigOff, byte[] pk, byte[] seed, byte[] masks, byte[] m_hash)
		{
		...
		internal static int horst_verify(HashFunctions hs, byte[] pk, byte[] sig, int sigOff, byte[] masks, byte[] m_hash)
		{
			...
			for (i = 0; i < HORST_K; i++)
			{
				...
				{
					idx = (int)((uint)idx >> 1); // parent node
