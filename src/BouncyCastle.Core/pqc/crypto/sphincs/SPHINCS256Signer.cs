...
namespace org.bouncycastle.pqc.crypto.sphincs
{
	...
	/// </para>
	/// </summary>
	public class SPHINCS256Signer : MessageSigner
	{
		...
		/// Base constructor.
		/// </summary>
		/// <param name="nDigest">  the "n-digest" must produce 32 bytes of output - used for tree construction. </param>
		/// <param name="twoNDigest"> the "2n-digest" must produce 64 bytes of output - used for initial message/key/seed hashing. </param>
		...
		internal static void validate_authpath(HashFunctions hs, byte[] root, byte[] leaf, int leafidx, byte[] authpath, int auOff, byte[] masks, int height)
		{
			int i, j;
		...
		internal static void compute_authpath_wots(HashFunctions hs, byte[] root, byte[] authpath, int authOff, Tree.leafaddr a, byte[] sk, byte[] masks, int height)
		{
			int i, idx, j;
			...
			for (ta.subleaf = 0; ta.subleaf < (1 << SPHINCS256Config.SUBTREE_HEIGHT); ta.subleaf++)
			{
				Tree.l_tree(hs, tree, (int)((1 << SPHINCS256Config.SUBTREE_HEIGHT) * SPHINCS256Config.HASH_BYTES + ta.subleaf * SPHINCS256Config.HASH_BYTES), pk, (int)(ta.subleaf * Wots.WOTS_L * SPHINCS256Config.HASH_BYTES), masks, 0);
			}

			int level = 0;

			// tree
			for (i = (1 << SPHINCS256Config.SUBTREE_HEIGHT); i > 0; i = (int)((uint)i >> 1))
			{
				for (j = 0; j < i; j += 2)
				{
					hs.hash_2n_n_mask(tree, ((int)((uint)i >> 1)) * SPHINCS256Config.HASH_BYTES + ((int)((uint)j >> 1)) * SPHINCS256Config.HASH_BYTES, tree, i * SPHINCS256Config.HASH_BYTES + j * SPHINCS256Config.HASH_BYTES, masks, 2 * (Wots.WOTS_LOG_L + level) * SPHINCS256Config.HASH_BYTES);
				}
			...
			for (i = 0; i < height; i++)
			{
				JavaSystem.arraycopy(tree, ((int)((uint)(1 << SPHINCS256Config.SUBTREE_HEIGHT) >> i)) * SPHINCS256Config.HASH_BYTES + (((int)((uint)idx >> i)) ^ 1) * SPHINCS256Config.HASH_BYTES, authpath, authOff + i * SPHINCS256Config.HASH_BYTES, SPHINCS256Config.HASH_BYTES);
			}

			// copy root
			JavaSystem.arraycopy(tree, SPHINCS256Config.HASH_BYTES, root, 0, SPHINCS256Config.HASH_BYTES);
		}

		public virtual byte[] crypto_sign(HashFunctions hs, byte[] m, byte[] sk)
		{
			...
			// create leafidx deterministically
				...
				JavaSystem.arraycopy(tsk, SPHINCS256Config.CRYPTO_SECRETKEYBYTES - SPHINCS256Config.SK_RAND_SEED_BYTES, sm, scratch, SPHINCS256Config.SK_RAND_SEED_BYTES);
				...
				JavaSystem.arraycopy(bRnd, 16, R, 0, SPHINCS256Config.MESSAGE_HASH_SEED_BYTES);

				// prepare msg_hash
				scratch = SPHINCS256Config.CRYPTO_BYTES - SPHINCS256Config.MESSAGE_HASH_SEED_BYTES - SPHINCS256Config.CRYPTO_PUBLICKEYBYTES;

				// cpy R
				JavaSystem.arraycopy(R, 0, sm, scratch, SPHINCS256Config.MESSAGE_HASH_SEED_BYTES);
				...
				JavaSystem.arraycopy(tsk, SPHINCS256Config.SEED_BYTES, sm, pk, Horst.N_MASKS * SPHINCS256Config.HASH_BYTES);
			...
			a.subtree = (long)((ulong)leafidx >> SPHINCS256Config.SUBTREE_HEIGHT);
			...
			JavaSystem.arraycopy(tsk, SPHINCS256Config.SEED_BYTES, masks, 0, Horst.N_MASKS * SPHINCS256Config.HASH_BYTES);
			for (i = 0; i < (SPHINCS256Config.TOTALTREE_HEIGHT + 7) / 8; i++)
			{
				sm[smOff + i] = unchecked((byte)(((long)((ulong)leafidx >> 8 * i)) & 0xff));
			}
			...
			for (i = 0; i < SPHINCS256Config.N_LEVELS; i++)
			{
				...
				a.subtree = (long)((ulong)a.subtree >> SPHINCS256Config.SUBTREE_HEIGHT);
			}
		...
		public virtual bool verify(HashFunctions hs, byte[] m, byte[] sm, byte[] pk)
		{
			int i;
			int smlen = sm.Length;
			long leafidx = 0;
			...
			// construct message hash
				...
				JavaSystem.arraycopy(sm, 0, sig, 0, SPHINCS256Config.CRYPTO_BYTES);
