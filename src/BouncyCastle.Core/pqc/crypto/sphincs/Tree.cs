...
namespace org.bouncycastle.pqc.crypto.sphincs
{
	public class Tree
	{
		public class leafaddr
		{
			internal int level;
			internal long subtree;
			internal long subleaf;
		...
		internal static void l_tree(HashFunctions hs, byte[] leaf, int leafOff, byte[] wots_pk, int pkOff, byte[] masks, int masksOff)
		{
			...
			for (i = 0; i < Wots.WOTS_LOG_L; i++)
			{
				...
				if ((l & 1) != 0)
				{
					JavaSystem.arraycopy(wots_pk, pkOff + (l - 1) * SPHINCS256Config.HASH_BYTES, wots_pk, pkOff + ((int)((uint)l >> 1)) * SPHINCS256Config.HASH_BYTES, SPHINCS256Config.HASH_BYTES);
					l = ((int)((uint)l >> 1)) + 1;
				}
				else
				{
			...
			JavaSystem.arraycopy(wots_pk, pkOff, leaf, leafOff, SPHINCS256Config.HASH_BYTES);
		}
		...
		internal static void treehash(HashFunctions hs, byte[] node, int nodeOff, int height, byte[] sk, leafaddr leaf, byte[] masks, int masksOff)
		{
			...
			for (; a.subleaf < lastnode; a.subleaf++)
			{
				...
				while (stackoffset > 1 && stacklevels[stackoffset - 1] == stacklevels[stackoffset - 2])
				{
					...
					hs.hash_2n_n_mask(stack, (stackoffset - 2) * SPHINCS256Config.HASH_BYTES, stack, (stackoffset - 2) * SPHINCS256Config.HASH_BYTES, masks, masksOff + maskoffset);
		...
		internal static void gen_leaf_wots(HashFunctions hs, byte[] leaf, int leafOff, byte[] masks, int masksOff, byte[] sk, leafaddr a)
		{
