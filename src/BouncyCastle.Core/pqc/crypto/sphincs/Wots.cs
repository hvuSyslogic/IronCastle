namespace org.bouncycastle.pqc.crypto.sphincs
{
	public class Wots
	{
		internal const int WOTS_LOGW = 4;
		...
		internal const int WOTS_L = 67; // for WOTS_W == 16
		internal const int WOTS_LOG_L = 7; // for WOTS_W == 16
		internal static readonly int WOTS_SIGBYTES = (WOTS_L * SPHINCS256Config.HASH_BYTES);
		...
		internal static void gen_chain(HashFunctions hs, byte[] @out, int outOff, byte[] seed, int seedOff, byte[] masks, int masksOff, int chainlen)
		{
			int i, j;
		...
		public virtual void wots_pkgen(HashFunctions hs, byte[] pk, int pkOff, byte[] sk, int skOff, byte[] masks, int masksOff)
		{
			int i;
		...
		public virtual void wots_sign(HashFunctions hs, byte[] sig, int sigOff, byte[] msg, byte[] sk, byte[] masks)
		{
			...
			{
				basew[i] = msg[i / 2] & 0xf;
				basew[i + 1] = (int)((uint)(msg[i / 2] & 0xff) >> 4);
				c += WOTS_W - 1 - basew[i];
		...
		public virtual void wots_verify(HashFunctions hs, byte[] pk, byte[] sig, int sigOff, byte[] msg, byte[] masks)
		{
			int[] basew = new int[WOTS_L];
			int i, c = 0;
