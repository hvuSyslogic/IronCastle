namespace org.bouncycastle.pqc.crypto.newhope
{
	...
	public class Poly
	{
		...
		internal static void fromBytes(short[] r, byte[] a)
		{
			for (int i = 0; i < Params.N / 4; ++i)
			{
				...
				r[k + 1] = (short)(((int)((uint)a1 >> 6)) | (a2 << 2) | ((a3 & 0x0F) << 10));
				r[k + 2] = (short)(((int)((uint)a3 >> 4)) | (a4 << 4) | ((a5 & 0x03) << 12));
				r[k + 3] = (short)(((int)((uint)a5 >> 2)) | (a6 << 6));
			}
		}

		internal static void fromNTT(short[] r)
		{
			NTT.bitReverse(r);
		...
		internal static void getNoise(short[] r, byte[] seed, byte nonce)
		{
			byte[] iv = new byte[8];
			iv[0] = nonce;
			...
			for (int i = 0; i < Params.N; ++i)
			{
				...
				int a = (((int)((uint)d >> 24)) + ((int)((uint)d >> 0))) & 0xFF;
				int b = (((int)((uint)d >> 16)) + ((int)((uint)d >> 8))) & 0xFF;
				r[i] = (short)(a + Params.Q - b);
			}
		}

		internal static void pointWise(short[] x, short[] y, short[] z)
		{
			for (int i = 0; i < Params.N; ++i)
			{
		...
		internal static void toBytes(byte[] r, short[] p)
		{
			for (int i = 0; i < Params.N / 4; ++i)
			{
				int j = 4 * i;
