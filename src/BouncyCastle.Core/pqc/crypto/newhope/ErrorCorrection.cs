namespace org.bouncycastle.pqc.crypto.newhope
{
	...
	public class ErrorCorrection
	{
		...
		internal static int f(int[] v, int off0, int off1, int x)
		{
			int xit, t, r, b;
		...
		internal static void helpRec(short[] c, short[] v, byte[] seed, byte nonce)
		{
			byte[] iv = new byte[8];
	//        iv[7] = nonce;
			iv[0] = nonce;
	...
	//      int32_t v0[4], v1[4], v_tmp[4], k;
			...
			for (int i = 0; i < 256; ++i)
			{
				int rBit = ((int)((uint)rand[(int)((uint)i >> 3)] >> (i & 7))) & 1;
		...
		internal static short LDDecode(int xi0, int xi1, int xi2, int xi3)
		{
			int t;
