...
namespace org.bouncycastle.crypto.tls
{
	...
	public class TlsBlockCipher : TlsCipher
	{
		...
		protected internal byte[] randomData;
		protected internal bool useExplicitIV;
		protected internal bool encryptThenMAC;

		protected internal BlockCipher encryptCipher;
		protected internal BlockCipher decryptCipher;

		protected internal TlsMac writeMac;
		protected internal TlsMac readMac;

		public virtual TlsMac getWriteMac()
		{
			return writeMac;
		}

		public virtual TlsMac getReadMac()
		{
			return readMac;
		}
		...
		public virtual byte[] encodePlaintext(long seqNo, short type, byte[] plaintext, int offset, int len)
		{
			...
			if (useExplicitIV)
			{
				...
				JavaSystem.arraycopy(explicitIV, 0, outBuf, outOff, blockSize);
				outOff += blockSize;
			}

			int blocks_start = outOff;

			JavaSystem.arraycopy(plaintext, offset, outBuf, outOff, len);
			outOff += len;

			if (!encryptThenMAC)
			{
				byte[] mac = writeMac.calculateMac(seqNo, type, plaintext, offset, len);
				JavaSystem.arraycopy(mac, 0, outBuf, outOff, mac.Length);
				outOff += mac.Length;
			}
	...
	//        assert outBuf.length == outOff;
		...
		public virtual int chooseExtraPadBlocks(SecureRandom r, int max)
		{
		...
		public virtual int lowestBitSet(int x)
		{
			if (x == 0)
			{
				return 32;
