...
namespace org.bouncycastle.crypto.tls
{
	...
	public class TlsServerProtocol : TlsProtocol
	{
		...
		protected internal TlsCredentials serverCredentials = null;
		protected internal CertificateRequest certificateRequest = null;

		protected internal short clientCertificateType = -1;
		protected internal TlsHandshakeHash prepareFinishHash = null;

		/// <summary>
		/// Constructor for blocking mode. </summary>
		/// <param name="input"> The stream of data from the client </param>
		/// <param name="output"> The stream of data to the client </param>
		/// <param name="secureRandom"> Random number generator for various cryptographic functions </param>
		public TlsServerProtocol(InputStream input, OutputStream output, SecureRandom secureRandom) : base(input, output, secureRandom)
		{
		}
		...
		public virtual void accept(TlsServer tlsServer)
		{
			...
			this.securityParameters.serverRandom = createRandomBlock(tlsServer.shouldUseGMTUnixTime(), tlsServerContext.getNonceRandomGenerator());
		...
		public override void handleAlertWarningMessage(short alertDescription)
		{
		...
		public virtual void receiveCertificateMessage(ByteArrayInputStream buf)
		{
		...
		public virtual void receiveCertificateVerifyMessage(ByteArrayInputStream buf)
		{
			if (certificateRequest == null)
			{
				throw new IllegalStateException();
		...
		public virtual void receiveClientHelloMessage(ByteArrayInputStream buf)
		{
		...
		public virtual void receiveClientKeyExchangeMessage(ByteArrayInputStream buf)
		{
		...
		public virtual void sendCertificateRequestMessage(CertificateRequest certificateRequest)
		{
		...
		public virtual void sendCertificateStatusMessage(CertificateStatus certificateStatus)
		{
		...
		public virtual void sendNewSessionTicketMessage(NewSessionTicket newSessionTicket)
		{
		...
		public virtual void sendServerHelloMessage()
		{
			...
			if (!Arrays.contains(offeredCipherSuites, selectedCipherSuite) || selectedCipherSuite == CipherSuite.TLS_NULL_WITH_NULL_NULL || CipherSuite.isSCSV(selectedCipherSuite) || !TlsUtils.isValidCipherSuiteForVersion(selectedCipherSuite, getContext().getServerVersion()))
			{
				throw new TlsFatalAlert(AlertDescription.internal_error);
			}
			...
			if (!this.serverExtensions.isEmpty())
			{
				...
				this.securityParameters.maxFragmentLength = processMaxFragmentLengthExtension(clientExtensions, serverExtensions, AlertDescription.internal_error);
				 ...
				 */
				this.allowCertificateStatus = !resumedSession && TlsUtils.hasExpectedEmptyExtensionData(serverExtensions, TlsExtensionsUtils.EXT_status_request, AlertDescription.internal_error);

				this.expectSessionTicket = !resumedSession && TlsUtils.hasExpectedEmptyExtensionData(serverExtensions, TlsProtocol.EXT_SessionTicket, AlertDescription.internal_error);
		...
		public virtual void sendServerHelloDoneMessage()
		{
		...
		public virtual void sendServerKeyExchangeMessage(byte[] serverKeyExchange)
		{
		...
		public virtual bool expectCertificateVerifyMessage()
		{
