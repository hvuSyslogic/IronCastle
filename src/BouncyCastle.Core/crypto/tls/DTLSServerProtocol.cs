...
namespace org.bouncycastle.crypto.tls
{
	...
	{
		protected internal bool verifyRequests = true;

		public DTLSServerProtocol(SecureRandom secureRandom) : base(secureRandom)
		{
		}

		public virtual bool getVerifyRequests()
		{
			return verifyRequests;
		}

		public virtual void setVerifyRequests(bool verifyRequests)
		{
		...
		public virtual void abortServerHandshake(ServerHandshakeState state, DTLSRecordLayer recordLayer, short alertDescription)
		{
			recordLayer.fail(alertDescription);
			invalidateSession(state);
		}

		public virtual DTLSTransport serverHandshake(ServerHandshakeState state, DTLSRecordLayer recordLayer)
		{
	...
	//        }
		...
		public virtual byte[] generateCertificateRequest(ServerHandshakeState state, CertificateRequest certificateRequest)
		{
		...
		public virtual byte[] generateCertificateStatus(ServerHandshakeState state, CertificateStatus certificateStatus)
		{
			ByteArrayOutputStream buf = new ByteArrayOutputStream();
		...
		public virtual byte[] generateNewSessionTicket(ServerHandshakeState state, NewSessionTicket newSessionTicket)
		{
			ByteArrayOutputStream buf = new ByteArrayOutputStream();
		...
		public virtual byte[] generateServerHello(ServerHandshakeState state)
		{
			SecurityParameters securityParameters = state.serverContext.getSecurityParameters();
			...
			if (!Arrays.contains(state.offeredCipherSuites, selectedCipherSuite) || selectedCipherSuite == CipherSuite.TLS_NULL_WITH_NULL_NULL || CipherSuite.isSCSV(selectedCipherSuite) || !TlsUtils.isValidCipherSuiteForVersion(selectedCipherSuite, state.serverContext.getServerVersion()))
			{
				throw new TlsFatalAlert(AlertDescription.internal_error);
			}
			...
			if (!state.serverExtensions.isEmpty())
			{
				...
				securityParameters.maxFragmentLength = evaluateMaxFragmentLengthExtension(state.resumedSession, state.clientExtensions, state.serverExtensions, AlertDescription.internal_error);
				 ...
				 */
				state.allowCertificateStatus = !state.resumedSession && TlsUtils.hasExpectedEmptyExtensionData(state.serverExtensions, TlsExtensionsUtils.EXT_status_request, AlertDescription.internal_error);

				state.expectSessionTicket = !state.resumedSession && TlsUtils.hasExpectedEmptyExtensionData(state.serverExtensions, TlsProtocol.EXT_SessionTicket, AlertDescription.internal_error);
		...
		public virtual void invalidateSession(ServerHandshakeState state)
		{
		...
		public virtual void notifyClientCertificate(ServerHandshakeState state, Certificate clientCertificate)
		{
			if (state.certificateRequest == null)
			{
		...
		public virtual void processClientCertificate(ServerHandshakeState state, byte[] body)
		{
		...
		public virtual void processCertificateVerify(ServerHandshakeState state, byte[] body, TlsHandshakeHash prepareFinishHash)
		{
			if (state.certificateRequest == null)
			{
				throw new IllegalStateException();
		...
		public virtual void processClientHello(ServerHandshakeState state, byte[] body)
		{
			ByteArrayInputStream buf = new ByteArrayInputStream(body);
		...
		public virtual void processClientKeyExchange(ServerHandshakeState state, byte[] body)
		{
			ByteArrayInputStream buf = new ByteArrayInputStream(body);
		...
		public virtual void processClientSupplementalData(ServerHandshakeState state, byte[] body)
		{
			ByteArrayInputStream buf = new ByteArrayInputStream(body);
		...
		public virtual bool expectCertificateVerifyMessage(ServerHandshakeState state)
		{
		...
		public class ServerHandshakeState
		{
			...
			internal short[] offeredCompressionMethods = null;
