...
namespace org.bouncycastle.crypto.tls
{
	...
	public class DTLSClientProtocol : DTLSProtocol
	{
		...
		public virtual void abortClientHandshake(ClientHandshakeState state, DTLSRecordLayer recordLayer, short alertDescription)
		{
			recordLayer.fail(alertDescription);
			invalidateSession(state);
		}

		public virtual DTLSTransport clientHandshake(ClientHandshakeState state, DTLSRecordLayer recordLayer)
		{
			...
			{
				state.sessionParameters = (new SessionParameters.Builder()).setCipherSuite(securityParameters.getCipherSuite()).setCompressionAlgorithm(securityParameters.getCompressionAlgorithm()).setExtendedMasterSecret(securityParameters.isExtendedMasterSecret()).setMasterSecret(securityParameters.getMasterSecret()).setPeerCertificate(serverCertificate).setPSKIdentity(securityParameters.getPSKIdentity()).setSRPIdentity(securityParameters.getSRPIdentity()).setServerExtensions(state.serverExtensions).build();
		...
		public virtual byte[] generateCertificateVerify(ClientHandshakeState state, DigitallySigned certificateVerify)
		{
		...
		public virtual byte[] generateClientHello(ClientHandshakeState state, TlsClient client)
		{
		...
		public virtual byte[] generateClientKeyExchange(ClientHandshakeState state)
		{
			ByteArrayOutputStream buf = new ByteArrayOutputStream();
		...
		public virtual void invalidateSession(ClientHandshakeState state)
		{
		...
		public virtual void processCertificateRequest(ClientHandshakeState state, byte[] body)
		{
		...
		public virtual void processCertificateStatus(ClientHandshakeState state, byte[] body)
		{
		...
		public virtual byte[] processHelloVerifyRequest(ClientHandshakeState state, byte[] body)
		{
			ByteArrayInputStream buf = new ByteArrayInputStream(body);
	...
	//        reportServerVersion(state, server_version);
		...
		public virtual void processNewSessionTicket(ClientHandshakeState state, byte[] body)
		{
			ByteArrayInputStream buf = new ByteArrayInputStream(body);
		...
		public virtual Certificate processServerCertificate(ClientHandshakeState state, byte[] body)
		{
			ByteArrayInputStream buf = new ByteArrayInputStream(body);
		...
		public virtual void processServerHello(ClientHandshakeState state, byte[] body)
		{
			SecurityParameters securityParameters = state.clientContext.getSecurityParameters();
			...
			if (!Arrays.contains(state.offeredCipherSuites, selectedCipherSuite) || selectedCipherSuite == CipherSuite.TLS_NULL_WITH_NULL_NULL || CipherSuite.isSCSV(selectedCipherSuite) || !TlsUtils.isValidCipherSuiteForVersion(selectedCipherSuite, state.clientContext.getServerVersion()))
			{
				throw new TlsFatalAlert(AlertDescription.illegal_parameter);
			}
	...
	//                    throw new TlsFatalAlert(AlertDescription.illegal_parameter);
			...
			if (sessionServerExtensions != null && !sessionServerExtensions.isEmpty())
			{
				 ...
				 */
				state.allowCertificateStatus = !state.resumedSession && TlsUtils.hasExpectedEmptyExtensionData(sessionServerExtensions, TlsExtensionsUtils.EXT_status_request, AlertDescription.illegal_parameter);
		...
		public virtual void processServerKeyExchange(ClientHandshakeState state, byte[] body)
		{
			ByteArrayInputStream buf = new ByteArrayInputStream(body);
		...
		public virtual void processServerSupplementalData(ClientHandshakeState state, byte[] body)
		{
			ByteArrayInputStream buf = new ByteArrayInputStream(body);
		...
		public virtual void reportServerVersion(ClientHandshakeState state, ProtocolVersion server_version)
		{
		...
		protected internal static byte[] patchClientHelloWithCookie(byte[] clientHelloBody, byte[] cookie)
		{
			int sessionIDPos = 34;
			...
			JavaSystem.arraycopy(clientHelloBody, 0, patched, 0, cookieLengthPos);
			TlsUtils.checkUint8(cookie.Length);
			TlsUtils.writeUint8(cookie.Length, patched, cookieLengthPos);
			JavaSystem.arraycopy(cookie, 0, patched, cookiePos, cookie.Length);
			JavaSystem.arraycopy(clientHelloBody, cookiePos, patched, cookiePos + cookie.Length, clientHelloBody.Length - cookiePos);
		...
		public class ClientHandshakeState
		{
			...
			internal Hashtable clientExtensions = null;
			internal Hashtable serverExtensions = null;
