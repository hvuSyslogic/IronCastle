...
namespace org.bouncycastle.crypto.prng.drbg
{
	...
	public class CTRSP800DRBG : SP80090DRBG
	{
		...
		/// </para> </summary>
		/// <param name="engine"> underlying block cipher to use to support DRBG </param>
		/// <param name="keySizeInBits"> size of the key to use with the block cipher. </param>
		...
		private void CTR_DRBG_Update(byte[] seed, byte[] key, byte[] v)
		{
			...
			while (i * outLen < seed.Length)
			{
				...
				JavaSystem.arraycopy(outputBlock, 0, temp, i * outLen, bytesToCopy);
				++i;
			}

			XOR(temp, seed, temp, 0);

			JavaSystem.arraycopy(temp, 0, key, 0, key.Length);
			JavaSystem.arraycopy(temp, key.Length, v, 0, v.Length);
		}
		...
		private byte[] Block_Cipher_df(byte[] inputString, int bitLength)
		{
			...
			JavaSystem.arraycopy(K_BITS, 0, K, 0, K.Length);
			...
			while (i * outLen * 8 < _keySizeInBits + outLen * 8)
			{
				...
				JavaSystem.arraycopy(bccOut, 0, temp, i * outLen, bytesToCopy);
				++i;
			}

			byte[] X = new byte[outLen];
			JavaSystem.arraycopy(temp, 0, K, 0, K.Length);
			JavaSystem.arraycopy(temp, K.Length, X, 0, X.Length);
			...
			while (i * outLen < temp.Length)
			{
				...
				JavaSystem.arraycopy(X, 0, temp, i * outLen, bytesToCopy);
				i++;
			}
		...
		* 6. Return output_block. 
		...
		private void BCC(byte[] bccOut, byte[] k, byte[] iV, byte[] data)
		{
			...
			JavaSystem.arraycopy(chainingValue, 0, bccOut, 0, bccOut.Length);
		}
		...
		public virtual int generate(byte[] output, byte[] additionalInput, bool predictionResistant)
		{
			...
			for (int i = 0; i <= output.Length / @out.Length; i++)
			{
				...
				if (bytesToCopy != 0)
				{
					...
					JavaSystem.arraycopy(@out, 0, output, i * @out.Length, bytesToCopy);
				}
			}
		...
		private bool isTDEA(BlockCipher cipher)
		{
		...
		public virtual byte[] expandKey(byte[] key)
		{
			if (_isTDEA)
			{
		...
		/// </summary>
		/// <param name="keyMaster"> </param>
		/// <param name="keyOff"> </param>
		/// <param name="tmp"> </param>
		/// <param name="tmpOff"> </param>
		private void padKey(byte[] keyMaster, int keyOff, byte[] tmp, int tmpOff)
		{
			tmp[tmpOff + 0] = unchecked((byte)(keyMaster[keyOff + 0] & 0xfe));
			tmp[tmpOff + 1] = (byte)((keyMaster[keyOff + 0] << 7) | ((int)((uint)(keyMaster[keyOff + 1] & 0xfc) >> 1)));
			tmp[tmpOff + 2] = (byte)((keyMaster[keyOff + 1] << 6) | ((int)((uint)(keyMaster[keyOff + 2] & 0xf8) >> 2)));
			tmp[tmpOff + 3] = (byte)((keyMaster[keyOff + 2] << 5) | ((int)((uint)(keyMaster[keyOff + 3] & 0xf0) >> 3)));
			tmp[tmpOff + 4] = (byte)((keyMaster[keyOff + 3] << 4) | ((int)((uint)(keyMaster[keyOff + 4] & 0xe0) >> 4)));
			tmp[tmpOff + 5] = (byte)((keyMaster[keyOff + 4] << 3) | ((int)((uint)(keyMaster[keyOff + 5] & 0xc0) >> 5)));
			tmp[tmpOff + 6] = (byte)((keyMaster[keyOff + 5] << 2) | ((int)((uint)(keyMaster[keyOff + 6] & 0x80) >> 6)));
			tmp[tmpOff + 7] = (byte)(keyMaster[keyOff + 6] << 1);

			for (int i = tmpOff; i <= tmpOff + 7; i++)
			{
				int b = tmp[i];
				tmp[i] = unchecked((byte)((b & 0xfe) | ((((b >> 1) ^ (b >> 2) ^ (b >> 3) ^ (b >> 4) ^ (b >> 5) ^ (b >> 6) ^ (b >> 7)) ^ 0x01) & 0x01)));
			}
		}
	}
