...
namespace org.bouncycastle.crypto.modes
{
	...
	public class GCMBlockCipher : AEADBlockCipher
	{
		...
		public virtual void init(bool forEncryption, CipherParameters @params)
		{
			...
			if (nonce.Length == 12)
			{
				JavaSystem.arraycopy(nonce, 0, J0, 0, nonce.Length);
		...
		private void initCipher()
		{
			if (atLength > 0)
			{
				JavaSystem.arraycopy(S_at, 0, S_atPre, 0, BLOCK_SIZE);
			...
			if (atLengthPre > 0)
			{
				JavaSystem.arraycopy(S_atPre, 0, S, 0, BLOCK_SIZE);
			}
		}
		...
		public virtual int processByte(byte @in, byte[] @out, int outOff)
		{
			...
			if (++bufOff == bufBlock.Length)
			{
				...
				{
					JavaSystem.arraycopy(bufBlock, BLOCK_SIZE, bufBlock, 0, macSize);
					bufOff = macSize;
				}
				return BLOCK_SIZE;
			}
		...
		public virtual int processBytes(byte[] @in, int inOff, int len, byte[] @out, int outOff)
		{
			...
			if (forEncryption)
			{
				...
				if (len > 0)
				{
					JavaSystem.arraycopy(@in, inOff, bufBlock, 0, len);
					bufOff = len;
				}
			}
			else
		...
		public virtual int doFinal(byte[] @out, int outOff)
		{
			...
			if (atLength > atLengthPre)
			{
				...
				long c = (long)((ulong)((totalLength * 8) + 127) >> 7);
			...
			JavaSystem.arraycopy(tag, 0, macBlock, 0, macSize);

			if (forEncryption)
			{
				// Append T to the message
				JavaSystem.arraycopy(macBlock, 0, @out, outOff + bufOff, macSize);
				resultLen += macSize;
			}
			else
			{
				...
				JavaSystem.arraycopy(bufBlock, extra, msgMac, 0, macSize);
		...
		private void processBlock(byte[] buf, int bufOff, byte[] @out, int outOff)
		{
			...
			if (forEncryption)
			{
				...
				JavaSystem.arraycopy(ctrBlock, 0, @out, outOff, BLOCK_SIZE);
			}
			else
			{
		...
		private void processPartial(byte[] buf, int off, int len, byte[] @out, int outOff)
		{
			...
			JavaSystem.arraycopy(buf, off, @out, outOff, len);
			totalLength += len;
		}
