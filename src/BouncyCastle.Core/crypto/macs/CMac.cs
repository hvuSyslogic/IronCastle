...
namespace org.bouncycastle.crypto.macs
{
	...
	public class CMac : Mac
	{
		...
		/// </summary>
		/// <param name="cipher">        the cipher to be used as the basis of the MAC generation. </param>
		/// <param name="macSizeInBits"> the size of the MAC in bits, must be a multiple of 8 and &lt;= 128. </param>
		...
		private byte[] doubleLu(byte[] @in)
		{
			 ...
			 */
			int mask = (-carry) & 0xff;
			ret[@in.Length - 3] ^= (byte)(poly[1] & mask);
			ret[@in.Length - 2] ^= (byte)(poly[2] & mask);
			ret[@in.Length - 1] ^= (byte)(poly[3] & mask);
		...
		public virtual void validate(CipherParameters @params)
		{
			if (@params != null)
			{
