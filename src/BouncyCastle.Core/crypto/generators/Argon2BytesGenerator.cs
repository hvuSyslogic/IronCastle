...
namespace org.bouncycastle.crypto.generators
{
	...
	public class Argon2BytesGenerator
	{
		...
		private static readonly int ARGON2_QWORDS_IN_BLOCK = ARGON2_BLOCK_SIZE / 8;
		...
		/// </summary>
		/// <param name="parameters"> Argon2 configuration. </param>
		public virtual void init(Argon2Parameters parameters)
		{
		...
		public virtual int generateBytes(char[] password, byte[] @out)
		{
			return generateBytes(parameters.getCharToByteConverter().convert(password), @out);
		}

		public virtual int generateBytes(char[] password, byte[] @out, int outOff, int outLen)
		{
			return generateBytes(parameters.getCharToByteConverter().convert(password), @out, outOff, outLen);
		}

		public virtual int generateBytes(byte[] password, byte[] @out)
		{
			return generateBytes(password, @out, 0, @out.Length);
		}

		public virtual int generateBytes(byte[] password, byte[] @out, int outOff, int outLen)
		{
			...
			JavaSystem.arraycopy(result, 0, @out, outOff, outLen);
		...
		private void fillBlock(Block X, Block Y, Block currentBlock, bool withXor)
		{
			Block R = new Block();
			Block Z = new Block();
			...
			for (int i = 0; i < 8; i++)
			{
				...
				roundFunction(Z, 16 * i, 16 * i + 1, 16 * i + 2, 16 * i + 3, 16 * i + 4, 16 * i + 5, 16 * i + 6, 16 * i + 7, 16 * i + 8, 16 * i + 9, 16 * i + 10, 16 * i + 11, 16 * i + 12, 16 * i + 13, 16 * i + 14, 16 * i + 15);
			}
			...
			for (int i = 0; i < 8; i++)
			{
				...
				roundFunction(Z, 2 * i, 2 * i + 1, 2 * i + 16, 2 * i + 17, 2 * i + 32, 2 * i + 33, 2 * i + 48, 2 * i + 49, 2 * i + 64, 2 * i + 65, 2 * i + 80, 2 * i + 81, 2 * i + 96, 2 * i + 97, 2 * i + 112, 2 * i + 113);
		...
		{
			return (parameters.getType() == Argon2Parameters.ARGON2_i) || (parameters.getType() == Argon2Parameters.ARGON2_id && (position.pass == 0) && (position.slice < ARGON2_SYNC_POINTS / 2));
		}
		...
		private long getPseudoRandom(Position position, Block addressBlock, Block inputBlock, Block zeroBlock, int prevOffset, bool dataIndependentAddressing)
		{
			if (dataIndependentAddressing)
			{
		...
		{
			int refLane = (int)((((long)((ulong)pseudoRandom >> 32))) % parameters.getLanes());
		...
		private int getRefColumn(Position position, long pseudoRandom, bool sameLane)
		{
	...
	//        long relativePosition = pseudoRandom << 32 >>> 32;
		...
		private byte[] hash(byte[] input, int outputLength)
		{
			...
			else
			{
				...
				JavaSystem.arraycopy(outBuffer, 0, result, 0, blake2bLength / 2);
				...
				for (int i = 2; i <= r; i++, position += blake2bLength / 2)
				{
					...
					JavaSystem.arraycopy(outBuffer, 0, result, position, blake2bLength / 2);
				}
		...
		private void roundFunction(Block block, int v0, int v1, int v2, int v3, int v4, int v5, int v6, int v7, int v8, int v9, int v10, int v11, int v12, int v13, int v14, int v15)
		{
		...
		private static void addIntToLittleEndian(Digest digest, int n)
		{
			...
			digest.update((byte)((int)((uint)n >> 8)));
			digest.update((byte)((int)((uint)n >> 16)));
			digest.update((byte)((int)((uint)n >> 24)));
		}
		...
		private byte[] getInitialHashLong(byte[] initialHash, byte[] appendix)
		{
			...
			JavaSystem.arraycopy(initialHash, 0, initialHashLong, 0, ARGON2_PREHASH_DIGEST_LENGTH);
			JavaSystem.arraycopy(appendix, 0, initialHashLong, ARGON2_PREHASH_DIGEST_LENGTH, 4);
		...
		public class Block
		{
			...
			public virtual void fromBytes(byte[] input)
			{
			...
			public virtual void copyBlock(Block other)
			{
				JavaSystem.arraycopy(other.v, 0, v, 0, v.Length);
			}

			public virtual void xor(Block b1, Block b2)
			{
				for (int i = 0; i < v.Length; i++)
				{
					v[i] = b1.v[i] ^ b2.v[i];
			...
			public virtual void xor(Block b1, Block b2, Block b3)
			{
				for (int i = 0; i < v.Length; i++)
				{
			...
			public virtual void xorWith(Block other)
			{
				for (int i = 0; i < v.Length; i++)
				{
					v[i] = v[i] ^ other.v[i];
