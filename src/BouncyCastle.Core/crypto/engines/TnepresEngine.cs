...
namespace org.bouncycastle.crypto.engines
{
	...
	public sealed class TnepresEngine : SerpentEngineBase
	{
		...
		public override int[] makeWorkingKey(byte[] key)
		{
			//
			// pad key to 256 bits
			//
			...
			JavaSystem.arraycopy(kPad, 8, w, 0, 8);
		...
		public override void encryptBlock(byte[] input, int inOff, byte[] output, int outOff)
		{
		...
		/// <param name="output"> the array the output data will be copied into. </param>
		/// <param name="outOff"> the offset into the out array the output will start at. </param>
		public override void decryptBlock(byte[] input, int inOff, byte[] output, int outOff)
		{
