...
namespace org.bouncycastle.crypto.engines
{
	...
	public class CryptoProWrapEngine : GOST28147WrapEngine
	{
		public override void init(bool forWrapping, CipherParameters param)
		{
			...
			if (sBox != null)
			{
				base.init(forWrapping, new ParametersWithUKM(new ParametersWithSBox(kParam, sBox), pU.getUKM()));
			}
			else
			{
				base.init(forWrapping, new ParametersWithUKM(kParam, pU.getUKM()));
			}
		}
		...
		private static bool bitSet(byte v, int bitNo)
		{
			return (v & (1 << bitNo)) != 0;
		}
	}
