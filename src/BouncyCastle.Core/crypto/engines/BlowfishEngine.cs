...
namespace org.bouncycastle.crypto.engines
{
	...
	/// </summary>
	public sealed class BlowfishEngine : BlockCipher
	{
		...
		public void init(bool encrypting, CipherParameters @params)
		{
			...
			throw new IllegalArgumentException("invalid parameter passed to Blowfish init - " + @params.GetType().getName());
		}
		...
		private int F(int x)
		{
			return (((S0[((int)((uint)x >> 24))] + S1[((int)((uint)x >> 16)) & 0xff]) ^ S2[((int)((uint)x >> 8)) & 0xff]) + S3[x & 0xff]);
		}
		...
		private void setKey(byte[] key)
		{
			 ...
			 */
			JavaSystem.arraycopy(KS0, 0, S0, 0, SBOX_SK);
			JavaSystem.arraycopy(KS1, 0, S1, 0, SBOX_SK);
			JavaSystem.arraycopy(KS2, 0, S2, 0, SBOX_SK);
			JavaSystem.arraycopy(KS3, 0, S3, 0, SBOX_SK);
