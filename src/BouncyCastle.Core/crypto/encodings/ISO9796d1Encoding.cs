...
namespace org.bouncycastle.crypto.encodings
{
	...
	/// </summary>
	public class ISO9796d1Encoding : AsymmetricBlockCipher
	{
		...
		private static byte[] shadows = new byte[] {0xe, 0x3, 0x5, 0x8, 0x9, 0x4, 0x2, 0xf, 0x0, 0xd, 0xb, 0x6, 0x7, 0xa, 0xc, 0x1};
		private static byte[] inverse = new byte[] {0x8, 0xf, 0x6, 0x1, 0x5, 0x2, 0xb, 0xc, 0x3, 0x4, 0xd, 0xa, 0xe, 0x9, 0x0, 0x7};
		...
		/// pad bits.
		/// </summary>
		public virtual void setPadBits(int padBits)
		{
			if (padBits > 7)
			{
		...
		private byte[] encodeBlock(byte[] @in, int inOff, int inLen)
		{
			...
			for (int i = 0; i < t; i += z)
			{
				if (i > t - z)
				{
					JavaSystem.arraycopy(@in, inOff + inLen - (t - i), block, block.Length - t, t - i);
				}
				else
				{
					JavaSystem.arraycopy(@in, inOff, block, block.Length - (i + z), z);
				}
			}
			...
			for (int i = block.Length - 2 * t; i != block.Length; i += 2)
			{
				...
				block[i] = (byte)((shadows[(int)((uint)(val & 0xff) >> 4)] << 4) | shadows[val & 0x0f]);
				block[i + 1] = val;
			}
			...
			if (maxBit != 8)
			{
				block[0] &= (byte)((int)((uint)0xff >> maxBit));
				block[0] |= (byte)((int)((uint)0x80 >> maxBit));
			}
			else
			{
				block[0] = 0x00;
		...
		/// <exception cref="InvalidCipherTextException"> if the decrypted block is not a valid ISO 9796 bit string </exception>
		private byte[] decodeBlock(byte[] @in, int inOff, int inLen)
		{
			...
			block[block.Length - 1] = (byte)(((int)((uint)(block[block.Length - 1] & 0xff) >> 4)) | ((inverse[(block[block.Length - 2] & 0xff) >> 4]) << 4));
			block[0] = (byte)((shadows[(int)((uint)(block[1] & 0xff) >> 4)] << 4) | shadows[block[1] & 0x0f]);
			...
			{
				int val = ((shadows[(int)((uint)(block[i] & 0xff) >> 4)] << 4) | shadows[block[i] & 0x0f]);
