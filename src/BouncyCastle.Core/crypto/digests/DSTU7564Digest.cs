...
namespace org.bouncycastle.crypto.digests
{
	...
	public class DSTU7564Digest : ExtendedDigest, Memoable
	{
		...
		public virtual int doFinal(byte[] @out, int outOff)
		{
			...
			// Apply padding: terminator byte and 96-bit length field
				...
				bufOff += 4;
				c = (long)((ulong)c >> 32);
				c += (((long)((ulong)inputBlocks >> 32)) * blockSize) << 3;
	...
	//            ^ rotate(56, m7);
			...
			long x1 = ((c & 0x7F7F7F7F7F7F7F7FL) << 1) ^ (((int)((uint)(c & 0x8080808080808080L) >> 7)) * 0x1DL);
			long u, v;
			...
			v = ((v & 0x3F3F3F3F3F3F3F3FL) << 2) ^ (((int)((uint)(v & 0x8080808080808080L) >> 6)) * 0x1DL) ^ (((long)((ulong)(v & 0x4040404040404040L) >> 6)) * 0x1DL);
		...
		private void shiftRows(long[] s)
		{
			...
			case NB_1024:
			{
				...
				d = (c00 ^ c08) & unchecked((long)0xFF00000000000000L);
				c00 ^= d;
				c08 ^= d;
				d = (c01 ^ c09) & unchecked((long)0xFF00000000000000L);
				c01 ^= d;
				c09 ^= d;
				d = (c02 ^ c10) & unchecked((long)0xFFFF000000000000L);
				c02 ^= d;
				c10 ^= d;
				d = (c03 ^ c11) & unchecked((long)0xFFFFFF0000000000L);
				c03 ^= d;
				c11 ^= d;
				d = (c04 ^ c12) & unchecked((long)0xFFFFFFFF00000000L);
				c04 ^= d;
				c12 ^= d;
				d = (c05 ^ c13) & 0x00FFFFFFFF000000L;
				c05 ^= d;
				c13 ^= d;
				d = (c06 ^ c14) & 0x00FFFFFFFFFF0000L;
				c06 ^= d;
				c14 ^= d;
				d = (c07 ^ c15) & 0x00FFFFFFFFFFFF00L;
				c07 ^= d;
				c15 ^= d;

				d = (c00 ^ c04) & 0x00FFFFFF00000000L;
				c00 ^= d;
				c04 ^= d;
				d = (c01 ^ c05) & unchecked((long)0xFFFFFFFFFF000000L);
				c01 ^= d;
				c05 ^= d;
				d = (c02 ^ c06) & unchecked((long)0xFF00FFFFFFFF0000L);
				c02 ^= d;
				c06 ^= d;
				d = (c03 ^ c07) & unchecked((long)0xFF0000FFFFFFFF00L);
				c03 ^= d;
				c07 ^= d;
				d = (c08 ^ c12) & 0x00FFFFFF00000000L;
				c08 ^= d;
				c12 ^= d;
				d = (c09 ^ c13) & unchecked((long)0xFFFFFFFFFF000000L);
				c09 ^= d;
				c13 ^= d;
				d = (c10 ^ c14) & unchecked((long)0xFF00FFFFFFFF0000L);
				c10 ^= d;
				c14 ^= d;
				d = (c11 ^ c15) & unchecked((long)0xFF0000FFFFFFFF00L);
				c11 ^= d;
				c15 ^= d;

				d = (c00 ^ c02) & unchecked((long)0xFFFF0000FFFF0000L);
				c00 ^= d;
				c02 ^= d;
				d = (c01 ^ c03) & 0x00FFFF0000FFFF00L;
				c01 ^= d;
				c03 ^= d;
				d = (c04 ^ c06) & unchecked((long)0xFFFF0000FFFF0000L);
				c04 ^= d;
				c06 ^= d;
				d = (c05 ^ c07) & 0x00FFFF0000FFFF00L;
				c05 ^= d;
				c07 ^= d;
				d = (c08 ^ c10) & unchecked((long)0xFFFF0000FFFF0000L);
				c08 ^= d;
				c10 ^= d;
				d = (c09 ^ c11) & 0x00FFFF0000FFFF00L;
				c09 ^= d;
				c11 ^= d;
				d = (c12 ^ c14) & unchecked((long)0xFFFF0000FFFF0000L);
				c12 ^= d;
				c14 ^= d;
				...
				d = (c00 ^ c01) & unchecked((long)0xFF00FF00FF00FF00L);
				c00 ^= d;
				c01 ^= d;
				d = (c02 ^ c03) & unchecked((long)0xFF00FF00FF00FF00L);
				c02 ^= d;
				c03 ^= d;
				d = (c04 ^ c05) & unchecked((long)0xFF00FF00FF00FF00L);
				c04 ^= d;
				c05 ^= d;
				d = (c06 ^ c07) & unchecked((long)0xFF00FF00FF00FF00L);
				c06 ^= d;
				c07 ^= d;
				d = (c08 ^ c09) & unchecked((long)0xFF00FF00FF00FF00L);
				c08 ^= d;
				c09 ^= d;
				d = (c10 ^ c11) & unchecked((long)0xFF00FF00FF00FF00L);
				c10 ^= d;
				c11 ^= d;
				d = (c12 ^ c13) & unchecked((long)0xFF00FF00FF00FF00L);
				c12 ^= d;
				c13 ^= d;
				d = (c14 ^ c15) & unchecked((long)0xFF00FF00FF00FF00L);
				c14 ^= d;
				c15 ^= d;
