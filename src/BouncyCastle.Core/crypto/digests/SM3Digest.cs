...
namespace org.bouncycastle.crypto.digests
{
	...
	public class SM3Digest : GeneralDigest
	{
		...
		private static readonly int[] T = new int[64];
		...
		static SM3Digest()
		{
			...
			{
				int t = 0x79CC4519;
				T[i] = (t << i) | ((int)((uint)t >> (32 - i)));
			}
			for (int i = 16; i < 64; ++i)
			{
				int n = i % 32;
				int t = 0x7A879D8A;
				T[i] = (t << n) | ((int)((uint)t >> (32 - n)));
			}
		}
		...
		{
			JavaSystem.arraycopy(t.V, 0, this.V, 0, this.V.Length);
			JavaSystem.arraycopy(t.inwords, 0, this.inwords, 0, this.inwords.Length);
			xOff = t.xOff;
		}
		...
		public override void reset()
		{
			...
			this.V[1] = 0x4914B2B9;
			this.V[2] = 0x172442D7;
			this.V[3] = unchecked((int)0xDA8A0600);
			this.V[4] = unchecked((int)0xA96F30BC);
			this.V[5] = 0x163138AA;
			this.V[6] = unchecked((int)0xE38DEE4D);
			this.V[7] = unchecked((int)0xB0FB0E4E);
		...
		public override void processLength(long bitLength)
		{
			...
			this.inwords[this.xOff++] = (int)((long)((ulong)bitLength >> 32));
	 ...
	 */
		...
		public override void processBlock()
		{
			...
			{
				int wj3 = this.W[j - 3];
				int r15 = ((wj3 << 15) | ((int)((uint)wj3 >> (32 - 15))));
				int wj13 = this.W[j - 13];
				int r7 = ((wj13 << 7) | ((int)((uint)wj13 >> (32 - 7))));
			...
			for (int j = 0; j < 16; ++j)
			{
				int a12 = ((A << 12) | ((int)((uint)A >> (32 - 12))));
				int s1_ = a12 + E + T[j];
				int SS1 = ((s1_ << 7) | ((int)((uint)s1_ >> (32 - 7))));
				int SS2 = SS1 ^ a12;
				int Wj = W[j];
				int W1j = Wj ^ W[j + 4];
				...
				D = C;
				C = ((B << 9) | ((int)((uint)B >> (32 - 9))));
				B = A;
				A = TT1;
				H = G;
				G = ((F << 19) | ((int)((uint)F >> (32 - 19))));
				F = E;
				E = P0(TT2);
			}
