...
namespace org.bouncycastle.crypto
{
	...
	/// </summary>
	public sealed class CryptoServicesRegistrar
	{
		private static readonly Permission CanSetDefaultProperty = new CryptoServicesPermission(CryptoServicesPermission.GLOBAL_CONFIG);
		private static readonly Permission CanSetThreadProperty = new CryptoServicesPermission(CryptoServicesPermission.THREAD_LOCAL_CONFIG);
		private static readonly Permission CanSetDefaultRandom = new CryptoServicesPermission(CryptoServicesPermission.DEFAULT_RANDOM);

		private static readonly ThreadLocal<Map<string, Object[]>> threadProperties = new ThreadLocal<Map<string, Object[]>>();
		private static readonly Map<string, Object[]> globalProperties = Collections.synchronizedMap(new HashMap<string, Object[]>());

		private static volatile SecureRandom defaultSecureRandom;

		static CryptoServicesRegistrar()
		{
		...
		/// </summary>
		/// <returns> the default SecureRandom </returns>
		/// <exception cref="IllegalStateException"> if no source of randomness has been provided. </exception>
		...
		/// </summary>
		/// <param name="secureRandom"> the SecureRandom to use as the default. </param>
		...
		/// @param <T> the type to be returned </param>
		/// <returns> null if the property is not set, the default value otherwise, </returns>
		public static T getProperty<T>(Property property)
		{
		...
		/// @param <T> the base type of the array to be returned. </param>
		/// <returns> null if the property is not set, an array of the current values otherwise. </returns>
		...
		/// </summary>
		/// <param name="property"> the name of the property to look up. </param>
		/// <param name="size"> the size (in bits) of the defining value in the property type. </param>
		/// @param <T> the type of the value to be returned. </param>
		/// <returns> the current value for the size, null if there is no value set, </returns>
		public static T getSizedProperty<T>(Property property, int size)
		{
			object[] values = lookupProperty(property);
		...
		/// <param name="propertyValue"> the values to assign to the property. </param>
		/// @param <T> the base type of the property value. </param>
		public static void setThreadProperty<T>(Property property, params T[] propertyValue)
		{
			checkPermission(CanSetThreadProperty);

			if (!property.type.isAssignableFrom(propertyValue[0].GetType()))
			{
		...
		/// <param name="propertyValue"> the values to assign to the property. </param>
		/// @param <T> the base type of the property value. </param>
		public static void setGlobalProperty<T>(Property property, params T[] propertyValue)
		{
		...
		private static void localSetThread<T>(Property property, T[] propertyValue)
		{
			Map<string, Object[]> properties = threadProperties.get();
		...
		private static void localSetGlobalProperty<T>(Property property, params T[] propertyValue)
		{
			if (!property.type.isAssignableFrom(propertyValue[0].GetType()))
			{
				throw new IllegalArgumentException("Bad property value passed");
		...
		/// </summary>
		/// <param name="property"> the property to be cleared. </param>
		/// @param <T> the base type of the property value </param>
		/// <returns> an array of T if a value was previously set, null otherwise. </returns>
		public static T[] clearGlobalProperty<T>(Property property)
		{
			checkPermission(CanSetDefaultProperty);
		...
		/// @param <T> the base type of the property value </param>
		/// <returns> an array of T if a value was previously set, null otherwise. </returns>
		public static T[] clearThreadProperty<T>(Property property)
		{
			checkPermission(CanSetThreadProperty);
		...
		private static void checkPermission(Permission permission)
		{
			...
			{
				AccessController.doPrivileged(new PrivilegedActionAnonymousInnerClass(permission, securityManager));
			}
		}

		public class PrivilegedActionAnonymousInnerClass : PrivilegedAction<object>
		{
			private Permission permission;
			private SecurityManager securityManager;

			public PrivilegedActionAnonymousInnerClass(Permission permission, SecurityManager securityManager)
			{
		...
		public sealed class Property
		{
			...
			/// </summary>
			public static readonly Property EC_IMPLICITLY_CA = new Property("ecImplicitlyCA", typeof(X9ECParameters));
			/// <summary>
			/// The default parameters for a particular size of Diffie-Hellman key.This is a sized property.
			/// </summary>
			public static readonly Property DH_DEFAULT_PARAMS = new Property("dhDefaultParams", typeof(DHParameters));
			/// <summary>
			/// The default parameters for a particular size of DSA key. This is a sized property.
			/// </summary>
			public static readonly Property DSA_DEFAULT_PARAMS = new Property("dsaDefaultParams", typeof(DSAParameters));
