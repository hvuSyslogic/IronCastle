using BouncyCastle.Core.Port;
using org.bouncycastle.Port.java.lang;

namespace org.bouncycastle.asn1.x9
{


	/// <summary>
	/// ASN.1 def for Elliptic-Curve Field ID structure. See
	/// X9.62, for further details.
	/// </summary>
	public class X9FieldID : ASN1Object, X9ObjectIdentifiers
	{
		private ASN1ObjectIdentifier id;
		private ASN1Primitive parameters;

		/// <summary>
		/// Constructor for elliptic curves over prime fields
		/// <code>F<sub>2</sub></code>. </summary>
		/// <param name="primeP"> The prime <code>p</code> defining the prime field. </param>
		public X9FieldID(BigInteger primeP)
		{
			this.id = X9ObjectIdentifiers_Fields.prime_field;
			this.parameters = new ASN1Integer(primeP);
		}

		/// <summary>
		/// Constructor for elliptic curves over binary fields
		/// <code>F<sub>2<sup>m</sup></sub></code>. </summary>
		/// <param name="m">  The exponent <code>m</code> of
		/// <code>F<sub>2<sup>m</sup></sub></code>. </param>
		/// <param name="k1"> The integer <code>k1</code> where <code>x<sup>m</sup> +
		/// x<sup>k1</sup> + 1</code>
		/// represents the reduction polynomial <code>f(z)</code>. </param>
		public X9FieldID(int m, int k1) : this(m, k1, 0, 0)
		{
		}

		/// <summary>
		/// Constructor for elliptic curves over binary fields
		/// <code>F<sub>2<sup>m</sup></sub></code>. </summary>
		/// <param name="m">  The exponent <code>m</code> of
		/// <code>F<sub>2<sup>m</sup></sub></code>. </param>
		/// <param name="k1"> The integer <code>k1</code> where <code>x<sup>m</sup> +
		/// x<sup>k3</sup> + x<sup>k2</sup> + x<sup>k1</sup> + 1</code>
		/// represents the reduction polynomial <code>f(z)</code>. </param>
		/// <param name="k2"> The integer <code>k2</code> where <code>x<sup>m</sup> +
		/// x<sup>k3</sup> + x<sup>k2</sup> + x<sup>k1</sup> + 1</code>
		/// represents the reduction polynomial <code>f(z)</code>. </param>
		/// <param name="k3"> The integer <code>k3</code> where <code>x<sup>m</sup> +
		/// x<sup>k3</sup> + x<sup>k2</sup> + x<sup>k1</sup> + 1</code>
		/// represents the reduction polynomial <code>f(z)</code>.. </param>
		public X9FieldID(int m, int k1, int k2, int k3)
		{
			this.id = X9ObjectIdentifiers_Fields.characteristic_two_field;
			ASN1EncodableVector fieldIdParams = new ASN1EncodableVector();
			fieldIdParams.add(new ASN1Integer(m));

			if (k2 == 0)
			{
				if (k3 != 0)
				{
					throw new IllegalArgumentException("inconsistent k values");
				}

				fieldIdParams.add(X9ObjectIdentifiers_Fields.tpBasis);
				fieldIdParams.add(new ASN1Integer(k1));
			}
			else
			{
				if (k2 <= k1 || k3 <= k2)
				{
					throw new IllegalArgumentException("inconsistent k values");
				}

				fieldIdParams.add(X9ObjectIdentifiers_Fields.ppBasis);
				ASN1EncodableVector pentanomialParams = new ASN1EncodableVector();
				pentanomialParams.add(new ASN1Integer(k1));
				pentanomialParams.add(new ASN1Integer(k2));
				pentanomialParams.add(new ASN1Integer(k3));
				fieldIdParams.add(new DERSequence(pentanomialParams));
			}

			this.parameters = new DERSequence(fieldIdParams);
		}

		private X9FieldID(ASN1Sequence seq)
		{
			this.id = ASN1ObjectIdentifier.getInstance(seq.getObjectAt(0));
			this.parameters = seq.getObjectAt(1).toASN1Primitive();
		}

		public static X9FieldID getInstance(object obj)
		{
			if (obj is X9FieldID)
			{
				return (X9FieldID)obj;
			}

			if (obj != null)
			{
				return new X9FieldID(ASN1Sequence.getInstance(obj));
			}

			return null;
		}

		public virtual ASN1ObjectIdentifier getIdentifier()
		{
			return id;
		}

		public virtual ASN1Primitive getParameters()
		{
			return parameters;
		}

		/// <summary>
		/// Produce a DER encoding of the following structure.
		/// <pre>
		///  FieldID ::= SEQUENCE {
		///      fieldType       FIELD-ID.&amp;id({IOSet}),
		///      parameters      FIELD-ID.&amp;Type({IOSet}{&#64;fieldType})
		///  }
		/// </pre>
		/// </summary>
		public override ASN1Primitive toASN1Primitive()
		{
			ASN1EncodableVector v = new ASN1EncodableVector();

			v.add(this.id);
			v.add(this.parameters);

			return new DERSequence(v);
		}
	}

}