...
namespace org.bouncycastle.asn1
{
	...
	/// </summary>
	public class DLApplicationSpecific : ASN1ApplicationSpecific
	{
		public DLApplicationSpecific(bool isConstructed, int tag, byte[] octets) : base(isConstructed, tag, octets)
		{
		}
		...
		public DLApplicationSpecific(int tag, byte[] octets) : this(false, tag, octets)
		{
		}
		...
		public DLApplicationSpecific(int tag, ASN1Encodable @object) : this(true, tag, @object)
		{
		}
		...
		/// <param name="tag"> the tag number for this object. </param>
		/// <param name="object"> the object to be contained. </param>
		public DLApplicationSpecific(bool constructed, int tag, ASN1Encodable @object) : base(constructed || @object.toASN1Primitive().isConstructed(), tag, getEncoding(constructed, @object))
		{
		}

		private static byte[] getEncoding(bool @explicit, ASN1Encodable @object)
		{
			byte[] data = @object.toASN1Primitive().getEncoded(ASN1Encoding_Fields.DL);
		...
		public DLApplicationSpecific(int tagNo, ASN1EncodableVector vec) : base(true, tagNo, getEncodedVector(vec))
		{
		}
		...
		private static byte[] getEncodedVector(ASN1EncodableVector vec)
		{
			...
			for (int i = 0; i != vec.size(); i++)
			{
				try
				{
					bOut.write(((ASN1Object)vec.get(i)).getEncoded(ASN1Encoding_Fields.DL));
				}
				catch (IOException e)
				{
