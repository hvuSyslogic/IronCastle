...
namespace org.bouncycastle.asn1.ua
{
	...
	public class DSTU4145NamedCurves
	{
		...
		internal static readonly ECDomainParameters[] @params = new ECDomainParameters[10];
		internal static readonly ASN1ObjectIdentifier[] oids = new ASN1ObjectIdentifier[10];

		//All named curves have the following oid format: 1.2.804.2.1.1.1.1.3.1.1.2.X
		//where X is the curve number 0-9
		internal static readonly string oidBase = UAObjectIdentifiers_Fields.dstu4145le.getId() + ".2.";
		...
		public static ECDomainParameters getByOID(ASN1ObjectIdentifier oid)
		{
			...
			if (oidStr.StartsWith(oidBase, StringComparison.Ordinal))
			{
				int index = int.Parse(oidStr.Substring(oidStr.LastIndexOf('.') + 1));
