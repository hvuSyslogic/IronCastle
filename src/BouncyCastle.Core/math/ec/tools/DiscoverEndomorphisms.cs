...
namespace org.bouncycastle.math.ec.tools
{
	...
	public class DiscoverEndomorphisms
	{
		...
		public static void Main(string[] args)
		{
			if (args.Length < 1)
			{
				JavaSystem.err.println("Expected a list of curve names as arguments");
				return;
			}
		...
		public static void discoverEndomorphisms(X9ECParameters x9)
		{
			...
			if (ECAlgorithms.isFpCurve(c))
			{
				...
				if (c.getA().isZero() && characteristic.mod(ECConstants_Fields.THREE).Equals(ECConstants_Fields.ONE))
				{
					JavaSystem.@out.println("Curve has a 'GLV Type B' endomorphism with these parameters:");
		...
		private static void discoverEndomorphisms(string curveName)
		{
			...
			if (x9 == null)
			{
				JavaSystem.err.println("Unknown curve: " + curveName);
				return;
			}
			...
			if (ECAlgorithms.isFpCurve(c))
			{
				...
				if (c.getA().isZero() && characteristic.mod(ECConstants_Fields.THREE).Equals(ECConstants_Fields.ONE))
				{
					JavaSystem.@out.println("Curve '" + curveName + "' has a 'GLV Type B' endomorphism with these parameters:");
					printGLVTypeBParameters(x9);
				}
			}
		}
		...
		{
			// x^2 + x + 1 = 0 mod n
			BigInteger[] lambdas = solveQuadraticEquation(x9.getN(), ECConstants_Fields.ONE, ECConstants_Fields.ONE);
		...
		private static void printGLVTypeBParameters(X9ECParameters x9, BigInteger lambda, ECFieldElement[] betas)
		{
			...
			if (!isVectorBoundedBySqrt(v2, n) && areRelativelyPrime(v1[0], v1[1]))
			{
				...
				if (vw != null)
				{
					...
					if (!check.Equals(ECConstants_Fields.ONE))
					{
					 ...
					 */
					BigInteger sqrtN = isqrt(n.subtract(ECConstants_Fields.ONE)).add(ECConstants_Fields.ONE);
					...
					if (range != null)
					{
						for (BigInteger alpha = range[0]; alpha.compareTo(range[1]) <= 0; alpha = alpha.add(ECConstants_Fields.ONE))
						{
			...
			printProperty("(OPT) bits", Convert.ToString(bits));
		}
		...
		{
			return a.gcd(b).Equals(ECConstants_Fields.ONE);
		}
		...
		private static BigInteger[] extEuclidBezout(BigInteger[] ab)
		{
			...
			BigInteger s0 = ECConstants_Fields.ONE, s1 = ECConstants_Fields.ZERO;
			BigInteger t0 = ECConstants_Fields.ZERO, t1 = ECConstants_Fields.ONE;

			while (r1.compareTo(ECConstants_Fields.ONE) > 0)
			{
		...
		private static bool isLessThanSqrt(BigInteger a, BigInteger b)
		{
			a = a.abs();
			b = b.abs();
		...
		private static bool isShorter(BigInteger[] u, BigInteger[] v)
		{
			...
			bool c1 = u1.compareTo(v1) < 0, c2 = u2.compareTo(v2) < 0;
			if (c1 == c2)
			{
				return c1;
			}
		...
		private static bool isVectorBoundedBySqrt(BigInteger[] v, BigInteger n)
		{
		...
		{
			bool negative = (x.signum() != y.signum());
			x = x.abs();
			y = y.abs();
	...
	//        assert root2.testBit(0);
		...
		private static ECFieldElement[] findBetaValues(ECCurve c)
		{
			...
			BigInteger e = q.divide(ECConstants_Fields.THREE);
			...
			do
			{
				BigInteger r = BigIntegers.createRandomInRange(ECConstants_Fields.TWO, q.subtract(ECConstants_Fields.TWO), random);
				b = r.modPow(e, q);
			} while (b.Equals(ECConstants_Fields.ONE));
