...
namespace org.bouncycastle.math.ec.rfc7748
{
	...
	public abstract class X448
	{
		public class Friend
		{
			internal static readonly Friend INSTANCE = new Friend();
			public Friend()
			{
			}
		}

		public const int POINT_SIZE = 56;
		public const int SCALAR_SIZE = 56;

		private const int C_A = 156326;
		private static readonly int C_A24 = (C_A + 2) / 4;
	...
	//        0x0BA4D3A0 };
		...
		private static void decodeScalar(byte[] k, int kOff, int[] n)
		{
			...
			n[0] &= unchecked((int)0xFFFFFFFC);
			n[13] |= unchecked((int)0x80000000);
		}
	...
	//            X448Field.apm(t1, x3, x3, z3);
				...
				int word = (int)((uint)bit >> 5), shift = bit & 0x1F;
				int kt = ((int)((uint)n[word] >> shift)) & 1;
				swap ^= kt;
