...
namespace org.bouncycastle.math.ec
{
	...
	{
		protected internal static readonly ECFieldElement[] EMPTY_ZS = new ECFieldElement[0];

		protected internal static ECFieldElement[] getInitialZCoords(ECCurve curve)
		{
			...
			ECFieldElement one = curve.fromBigInteger(ECConstants_Fields.ONE);
		...
		protected internal ECCurve curve;
		protected internal ECFieldElement x;
		protected internal ECFieldElement y;
		protected internal ECFieldElement[] zs;

		protected internal bool withCompression;

		// Hashtable is (String -> PreCompInfo)
		protected internal Hashtable preCompTable = null;

		public ECPoint(ECCurve curve, ECFieldElement x, ECFieldElement y) : this(curve, x, y, getInitialZCoords(curve))
		{
		}

		public ECPoint(ECCurve curve, ECFieldElement x, ECFieldElement y, ECFieldElement[] zs)
		{
			this.curve = curve;
			this.x = x;
			this.y = y;
		...
		public abstract bool satisfiesCurveEquation();

		public virtual bool satisfiesOrder()
		{
			if (ECConstants_Fields.ONE.Equals(curve.getCofactor()))
			{
				return true;
			}
		...
		public virtual int getCurveCoordinateSystem()
		{
			// Cope with null curve, most commonly used by implicitlyCa
		...
		/// </summary>
		/// <returns> The affine x-coordinate of this point </returns>
		/// <exception cref="IllegalStateException"> if the point is not normalized </exception>
		public virtual ECFieldElement getAffineXCoord()
		{
			checkNormalized();
			return getXCoord();
		}
		...
		/// </summary>
		/// <returns> The affine y-coordinate of this point </returns>
		/// <exception cref="IllegalStateException"> if the point is not normalized </exception>
		public virtual ECFieldElement getAffineYCoord()
		{
			checkNormalized();
			return getYCoord();
		}
		...
		public virtual ECFieldElement getXCoord()
		{
			return x;
		}
		...
		public virtual ECFieldElement getYCoord()
		{
			return y;
		}
		...
		public virtual ECFieldElement[] getZCoords()
		{
			...
			JavaSystem.arraycopy(zs, 0, copy, 0, zsLen);
			return copy;
		}
		...
		public ECFieldElement[] getRawZCoords()
		{
			return zs;
		}

		public virtual void checkNormalized()
		{
			if (!isNormalized())
			{
		...
		public virtual bool isNormalized()
		{
			...
			return coord == ECCurve.COORD_AFFINE || coord == ECCurve.COORD_LAMBDA_AFFINE || isInfinity() || zs[0].isOne();
		}
		...
		/// </summary>
		/// <returns> a new ECPoint instance representing the same point, but with normalized coordinates </returns>
		...
		public virtual ECPoint normalize(ECFieldElement zInv)
		{
			switch (this.getCurveCoordinateSystem())
			{
			case ECCurve.COORD_HOMOGENEOUS:
		...
		public virtual ECPoint createScaledPoint(ECFieldElement sx, ECFieldElement sy)
		{
		...
		public virtual bool isValidPartial()
		{
		...
		public virtual bool implIsValid(bool decompressed, bool checkOrder)
		{
			...
			ValidityPrecompInfo validity = (ValidityPrecompInfo)getCurve().precompute(this, ValidityPrecompInfo.PRECOMP_NAME, new PreCompCallbackAnonymousInnerClass(this, decompressed, checkOrder));
		...
		{
			private readonly ECPoint outerInstance;

			private bool decompressed;
			private bool checkOrder;

			public PreCompCallbackAnonymousInnerClass(ECPoint outerInstance, bool decompressed, bool checkOrder)
			{
			...
			{
				ValidityPrecompInfo info = (existing is ValidityPrecompInfo) ? (ValidityPrecompInfo)existing : null;
				if (info == null)
				{
				...
				{
					if (!decompressed && !outerInstance.satisfiesCurveEquation())
					{
						info.reportFailed();
						return info;
					}
				...
				{
					if (!outerInstance.satisfiesOrder())
					{
						info.reportFailed();
						return info;
					}
		...
		public override int GetHashCode()
		{
			...
			if (!this.isInfinity())
			{
				...
				hc ^= p.getXCoord().GetHashCode() * 17;
				hc ^= p.getYCoord().GetHashCode() * 257;
			}
		...
		/// @deprecated per-point compression property will be removed, refer <seealso cref="#getEncoded(boolean)"/> 
		/// <returns> a byte encoding. </returns>
		...
		/// </summary>
		/// <param name="compressed"> whether to generate a compressed point encoding. </param>
		/// <returns> the point encoding </returns>
		...
		/// <summary>
		/// Multiplies this <code>ECPoint</code> by the given number. </summary>
		/// <param name="k"> The multiplicator. </param>
		/// <returns> <code>k * this</code>. </returns>
		public virtual ECPoint multiply(BigInteger k)
		{
			return this.getCurve().getMultiplier().multiply(this, k);
		}

		public abstract class AbstractFp : ECPoint
		{
			public AbstractFp(ECCurve curve, ECFieldElement x, ECFieldElement y) : base(curve, x, y)
			{
			}

			public AbstractFp(ECCurve curve, ECFieldElement x, ECFieldElement y, ECFieldElement[] zs) : base(curve, x, y, zs)
			{
			}

			public override bool getCompressionYTilde()
			{
				return this.getAffineYCoord().testBitZero();
			}

			public override bool satisfiesCurveEquation()
			{
		...
		public class Fp : AbstractFp
		{
			...
			/// </param>
			/// @deprecated per-point compression property will be removed, refer <seealso cref="#getEncoded(boolean)"/> 
			public Fp(ECCurve curve, ECFieldElement x, ECFieldElement y, bool withCompression) : base(curve, x, y)
			{
			...
			public Fp(ECCurve curve, ECFieldElement x, ECFieldElement y, ECFieldElement[] zs, bool withCompression) : base(curve, x, y, zs)
			{
	...
	//                ECFieldElement Z3 = doubleProductFromSquares(Y1, Z1, Y1Squared, Z1Squared);
			...
			public override ECPoint timesPow2(int e)
			{
				...
				ECFieldElement X1 = this.x;
				ECFieldElement Z1 = this.zs.Length < 1 ? curve.fromBigInteger(ECConstants_Fields.ONE) : this.zs[0];
			...
			public virtual ECFieldElement two(ECFieldElement x)
			{
				return x.add(x);
			}

			public virtual ECFieldElement three(ECFieldElement x)
			{
				return two(x).add(x);
			}

			public virtual ECFieldElement four(ECFieldElement x)
			{
				return two(two(x));
			}

			public virtual ECFieldElement eight(ECFieldElement x)
			{
				return four(two(x));
			}

			public virtual ECFieldElement doubleProductFromSquares(ECFieldElement a, ECFieldElement b, ECFieldElement aSquared, ECFieldElement bSquared)
			{
				/*
			...
			public virtual ECFieldElement calculateJacobianModifiedW(ECFieldElement Z, ECFieldElement ZSquared)
			{
			...
			public virtual ECFieldElement getJacobianModifiedW()
			{
			...
			public virtual ECPoint.Fp twiceJacobianModified(bool calculateW)
			{
		...
		public abstract class AbstractF2m : ECPoint
		{
			public AbstractF2m(ECCurve curve, ECFieldElement x, ECFieldElement y) : base(curve, x, y)
			{
			}

			public AbstractF2m(ECCurve curve, ECFieldElement x, ECFieldElement y, ECFieldElement[] zs) : base(curve, x, y, zs)
			{
			}
            ...
            public override bool satisfiesOrder()
			{
				...
				if (ECConstants_Fields.FOUR.Equals(cofactor))
				{
					/*
			...
			public virtual ECPoint.AbstractF2m tau()
			{
				if (this.isInfinity())
				{
					return this;
				...
				case ECCurve.COORD_LAMBDA_PROJECTIVE:
				{
					...
					return (ECPoint.AbstractF2m)curve.createRawPoint(X1.square(), Y1.square(), new ECFieldElement[]{Z1.square()}, this.withCompression);
				}
				default:
				{
					throw new IllegalStateException("unsupported coordinate system");
			...
			public virtual ECPoint.AbstractF2m tauPow(int pow)
			{
				if (this.isInfinity())
				{
					return this;
				...
				{
					ECFieldElement Y1 = this.y, Z1 = this.zs[0];
					return (ECPoint.AbstractF2m)curve.createRawPoint(X1.squarePow(pow), Y1.squarePow(pow), new ECFieldElement[]{Z1.squarePow(pow)}, this.withCompression);
				}
				default:
				{
					throw new IllegalStateException("unsupported coordinate system");
		...
		public class F2m : AbstractF2m
		{
			...
			/// <param name="withCompression"> true if encode with point compression.
			/// </param>
			/// @deprecated per-point compression property will be removed, refer <seealso cref="#getEncoded(boolean)"/> 
			public F2m(ECCurve curve, ECFieldElement x, ECFieldElement y, bool withCompression) : base(curve, x, y)
			{
	...
	//            checkCurveEquation();
			...
			public F2m(ECCurve curve, ECFieldElement x, ECFieldElement y, ECFieldElement[] zs, bool withCompression) : base(curve, x, y, zs)
			{
	...
	//            checkCurveEquation();
			...
			public override ECFieldElement getYCoord()
			{
				int coord = this.getCurveCoordinateSystem();
			...
			public override ECPoint add(ECPoint b)
			{
				...
				case ECCurve.COORD_LAMBDA_PROJECTIVE:
				{
					...
					if (X2.isZero())
					{
						...
						Z3 = curve.fromBigInteger(ECConstants_Fields.ONE);
					}
					else
					{
						B = B.square();
