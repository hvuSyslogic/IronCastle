...
namespace org.bouncycastle.math.ec.rfc8032
{
	...
	public abstract class Ed25519
	{
		...
		{
			private readonly Ed25519 outerInstance;

			public Algorithm(Ed25519 outerInstance)
			{
				this.outerInstance = outerInstance;
			}

			public const int Ed25519 = 0;
			public const int Ed25519ctx = 1;
			public const int Ed25519ph = 2;
		}

		private const long M28L = 0x0FFFFFFFL;
		private const long M32L = 0xFFFFFFFFL;

		private const int POINT_BYTES = 32;
		private const int SCALAR_INTS = 8;
		private static readonly int SCALAR_BYTES = SCALAR_INTS * 4;
		...
		private static readonly int[] B_x = new int[]{0x0325D51A, 0x018B5823, 0x007B2C95, 0x0304A92D, 0x00D2598E, 0x01D6DC5C, 0x01388C7F, 0x013FEC0A, 0x029E6B72, 0x0042D26D};
		private static readonly int[] B_y = new int[]{0x02666658, 0x01999999, 0x00666666, 0x03333333, 0x00CCCCCC, 0x02666666, 0x01999999, 0x00666666, 0x03333333, 0x00CCCCCC};
		private static readonly int[] C_d = new int[]{0x035978A3, 0x02D37284, 0x018AB75E, 0x026A0A0E, 0x0000E014, 0x0379E898, 0x01D01E5D, 0x01E738CC, 0x03715B7F, 0x00A406D9};
		private static readonly int[] C_d2 = new int[]{0x02B2F159, 0x01A6E509, 0x01156EBD, 0x00D4141D, 0x0001C029, 0x02F3D130, 0x03A03CBB, 0x01CE7198, 0x02E2B6FF, 0x00480DB3};
		private static readonly int[] C_d4 = new int[]{0x0165E2B2, 0x034DCA13, 0x002ADD7A, 0x01A8283B, 0x00038052, 0x01E7A260, 0x03407977, 0x019CE331, 0x01C56DFF, 0x00901B67};

		private const int WNAF_WIDTH_BASE = 7;

		private const int PRECOMP_BLOCKS = 8;
		private const int PRECOMP_TEETH = 4;
		private const int PRECOMP_SPACING = 8;
...
//ORIGINAL LINE: final int sign = pow2 >>> 1;
	...
	//                assert 0 <= abs && abs < PRECOMP_POINTS;
		...
		public static bool verifyPrehash(byte[] sig, int sigOff, byte[] pk, int pkOff, byte[] ctx, Digest ph)
		{
			byte[] m = new byte[PREHASH_SIZE];
			if (PREHASH_SIZE != ph.doFinal(m, 0))
			{
