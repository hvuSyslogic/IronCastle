...
namespace org.bouncycastle.math.ec
{
	...
	public class ECAlgorithms
	{
		...
		public static bool isF2mField(FiniteField field)
		{
			return field.getDimension() > 1 && field.getCharacteristic().Equals(ECConstants_Fields.TWO) && field is PolynomialExtensionField;
		}
		...
		public static bool isFpField(FiniteField field)
		{
		...
		/// </summary>
		/// <param name="p">
		///            The point to multiply. </param>
		/// <param name="k">
		///            The multiplier. </param>
		/// <returns> The result of the point multiplication <code>kP</code>. </returns>
		...
		{
			bool negK = k.signum() < 0, negL = l.signum() < 0;
		...
		private static ECPoint implShamirsTrickWNaf(ECPoint[] preCompP, ECPoint[] preCompNegP, byte[] wnafP, ECPoint[] preCompQ, ECPoint[] preCompNegQ, byte[] wnafQ)
		{
			...
			for (int i = len - 1; i >= 0; --i)
			{
				...
				if (wiP != 0)
				{
					...
					r = r.add(tableP[(int)((uint)nP >> 1)]);
				}
				if (wiQ != 0)
				{
					int nQ = Math.Abs(wiQ);
					ECPoint[] tableQ = wiQ < 0 ? preCompNegQ : preCompQ;
					r = r.add(tableQ[(int)((uint)nQ >> 1)]);
				}
