...
namespace org.bouncycastle.math
{
	...
	{
		public const int SMALL_FACTOR_LIMIT = 211;
		...
		/// {@link Primes#enhancedMRProbablePrimeTest(BigInteger, SecureRandom, int) Enhanced
		...
		public class MROutput
		{
			...
			internal bool provablyComposite;
			internal BigInteger factor;

			public MROutput(bool provablyComposite, BigInteger factor)
			{
				this.provablyComposite = provablyComposite;
				this.factor = factor;
			}

			public virtual BigInteger getFactor()
			{
				return factor;
			}

			public virtual bool isProvablyComposite()
			{
				return provablyComposite;
			}

			public virtual bool isNotPrimePower()
			{
		...
		/// <summary>
		/// Used to return the output from the
		/// {@link Primes#generateSTRandomPrime(Digest, int, byte[]) Shawe-Taylor Random_Prime
		/// Routine}
		/// </summary>
		public class STOutput
		{
			internal BigInteger prime;
			internal byte[] primeSeed;
			internal int primeGenCounter;

			public STOutput(BigInteger prime, byte[] primeSeed, int primeGenCounter)
			{
				this.prime = prime;
			...
			public virtual byte[] getPrimeSeed()
			{
				return primeSeed;
			}

			public virtual int getPrimeGenCounter()
			{
		...
		/// </summary>
		/// <param name="hash">
		///            the <seealso cref="Digest"/> instance to use (as "Hash()"). Cannot be null. </param>
		/// <param name="length">
		///            the length (in bits) of the prime to be generated. Must be at least 2. </param>
		/// <param name="inputSeed">
		///            the seed to be used for the generation of the requested prime. Cannot be null or
		///            empty. </param>
		/// <returns> an <seealso cref="STOutput"/> instance containing the requested prime. </returns>
		...
		/// alternative to <seealso cref="#isMRProbablePrime(BigInteger, SecureRandom, int)"/> that provides more
		/// information about a composite candidate, which may be useful when generating or validating
		/// RSA moduli.
		/// </summary>
		/// <param name="candidate">
		///            the <seealso cref="BigInteger"/> instance to test for primality. </param>
		/// <param name="random">
		///            the source of randomness to use to choose bases. </param>
		/// <param name="iterations">
		///            the number of randomly-chosen bases to perform the test for. </param>
		/// <returns> an <seealso cref="MROutput"/> instance that can be further queried for details. </returns>
		...
		/// </summary>
		/// <param name="candidate">
		///            the <seealso cref="BigInteger"/> instance to test for division by small factors.
		/// </param>
		/// <returns> <code>true</code> if the candidate is found to have any small factors,
		...
		///            the <seealso cref="BigInteger"/> instance to test for primality. </param>
		/// <param name="base">
		///            the base value to use for this iteration. </param>
		/// <returns> <code>false</code> if the specified base is a witness to compositeness (so
		///         <code>candidate</code> is definitely NOT prime), or else <code>true</code>. </returns>
		public static bool isMRProbablePrimeToBase(BigInteger candidate, BigInteger @base)
		{
			checkCandidate(candidate, "candidate");
		...
		private static STOutput implSTRandomPrime(Digest d, int length, byte[] primeSeed)
		{
			...
			if (length < 33)
			{
				...
				for (;;)
				{
					...
					c &= ((int)(unchecked((uint)-1) >> (32 - length)));
					c |= (1 << (length - 1)) | 1;
		...
		private static bool isPrime32(long x)
		{
			if ((long)((ulong)x >> 32) != 0L)
			{
