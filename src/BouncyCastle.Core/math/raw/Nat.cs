...
namespace org.bouncycastle.math.raw
{
	...
	public abstract class Nat
	{
		...
		public static int addDWordAt(int len, long x, int[] z, int zOff, int zPos)
		{
			...
			z[zOff + zPos] = (int)c;
			c = (long)((ulong)c >> 32);
			c += (z[zOff + zPos + 1] & M) + ((long)((ulong)x >> 32));
			z[zOff + zPos + 1] = (int)c;
			c = (long)((ulong)c >> 32);
			return c == 0 ? 0 : incAt(len, z, zOff, zPos + 2);
		}
	...
	//        }
		...
		{
			JavaSystem.arraycopy(x, xOff, z, zOff, len);
		}
		...
		public static bool eq(int len, int[] x, int[] y)
		{
		...
		public static bool gte(int len, int[] x, int[] y)
		{
			for (int i = len - 1; i >= 0; --i)
			{
		...
		public static bool isOne(int len, int[] x)
		{
			if (x[0] != 1)
			{
				return false;
		...
		public static bool isZero(int len, int[] x)
		{
			for (int i = 0; i < len; ++i)
			{
				if (x[i] != 0)
		...
		public static int mulWordDwordAddAt(int len, int x, long y, int[] z, int zPos)
		{
			...
			z[zPos + 0] = (int)c;
			c = (long)((ulong)c >> 32);
			c += xVal * ((long)((ulong)y >> 32)) + (z[zPos + 1] & M);
			z[zPos + 1] = (int)c;
			c = (long)((ulong)c >> 32);
			c += (z[zPos + 2] & M);
			z[zPos + 2] = (int)c;
	...
	//        assert bits > 0 && bits < 32;
			...
			{
				int next = z[zOff + i];
				z[zOff + i] = ((int)((uint)next >> bits)) | (c << -bits);
				c = next;
			}
			return c << -bits;
		}
	...
	//        assert bits > 0 && bits < 32;
		...
		public static int shiftUpBit(int len, int[] z, int c)
		{
			...
			{
				int next = z[i];
				z[i] = (next << 1) | ((int)((uint)c >> 31));
				c = next;
			}
			return (int)((uint)c >> 31);
		}
		...
		public static long shiftUpBit64(int len, long[] x, int xOff, long c, long[] z, int zOff)
		{
			...
			{
				long next = x[xOff + i];
				z[zOff + i] = (next << 1) | ((long)((ulong)c >> 63));
				c = next;
			}
			return (long)((ulong)c >> 63);
		}
	...
	//        assert bits > 0 && bits < 32;
			...
			{
				int next = z[i];
				z[i] = (next << bits) | ((int)((uint)c >> -bits));
				c = next;
			}
			return (int)((uint)c >> -bits);
		}
	...
	//        assert bits > 0 && bits < 32;
			...
			{
				int next = z[zOff + i];
				z[zOff + i] = (next << bits) | ((int)((uint)c >> -bits));
				c = next;
			}
			return (int)((uint)c >> -bits);
		}
	...
	//        assert bits > 0 && bits < 64;
			...
			{
				long next = z[zOff + i];
				z[zOff + i] = (next << bits) | ((long)((ulong)c >> -bits));
				c = next;
			}
			return (long)((ulong)c >> -bits);
		}
	...
	//        assert bits > 0 && bits < 64;
		...
		public static void square(int len, int[] x, int[] zz)
		{
			...
			do
			{
				long xVal = (x[--j] & M);
				long p = xVal * xVal;
				zz[--k] = (c << 31) | (int)((long)((ulong)p >> 33));
				zz[--k] = (int)((long)((ulong)p >> 1));
				c = (int)p;
			} while (j > 0);
		...
		public static void square(int len, int[] x, int xOff, int[] zz, int zzOff)
		{
			...
			do
			{
				...
				long p = xVal * xVal;
				zz[zzOff + --k] = (c << 31) | (int)((long)((ulong)p >> 33));
				zz[zzOff + --k] = (int)((long)((ulong)p >> 1));
				c = (int)p;
			} while (j > 0);
