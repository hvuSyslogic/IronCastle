	...
	public class ECAlgorithmParametersTest : TestCase
	{
		...
		public virtual void testRecogniseStandardCurveNames()
		{
			...
			for (int i = 0; i != entries.Length; i++)
			{
				...
				ecSpec = algParams.getParameterSpec(typeof(ECParameterSpec));


				ECGenParameterSpec spec = algParams.getParameterSpec(typeof(ECGenParameterSpec));
				...
				if (ecSpec != null)
				{
					...
					spec = algParams2.getParameterSpec(typeof(ECGenParameterSpec));
		...
		private string nextOid(int index)
		{
			...
			{
				if (entries[i][0] >= '0' && entries[i][0] <= '2')
				{
					return entries[i];
				}
			}
