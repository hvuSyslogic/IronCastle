...
namespace org.bouncycastle.jce.provider.test
{
	...
	public class CertTest : SimpleTest
	{
		...
		private readonly string ecPemCert = "-----BEGIN CERTIFICATE-----\n" +
		...
		private readonly string pemPKCS7 = "-----BEGIN PKCS7-----\n" +
	...
	//            JavaSystem.@out.println(cert);
		...
		public virtual void checkSelfSignedCertificate(int id, byte[] bytes, string sigAlgName)
		{
			ByteArrayInputStream bIn;
			string dump = "";
			...
			try
			{
				...
				cert.verify(k);
				if (!string.ReferenceEquals(sigAlgName, null) && !sigAlgName.Equals(((X509Certificate)cert).getSigAlgName()))
				{
		...
		public virtual void checkCertificateSignedBy(int id, byte[] certBytes, byte[] signingCertBytes)
		{
			ByteArrayInputStream bIn;
			string dump = "";
		...
		private void pemTest()
		{
			...
			if (!(new BigInteger("ffffffffffffffffffffffffffffffffffffffffffffffffc7634d81f4372ddf581a0db248b0a77aecec196accc52973", 16)).Equals(((ECPublicKey)cert.getPublicKey()).getParameters().getN()))
			{
				fail("N incorrect");
			}
		}
		...
		private void pkcs7Test()
		{
			...
			@in = new ByteArrayInputStream(pemPKCS7.GetBytes(Encoding.ASCII));
	...
	//        }
		...
		{
			byte[] certData = Streams.readAll(this.GetType().getResourceAsStream("ThawteSGCCA.cer"));
			byte[] crlData = Streams.readAll(this.GetType().getResourceAsStream("ThawteSGCCA.crl"));

			// verify CRL with default (JCE) provider
			CertificateFactory jceFac = CertificateFactory.getInstance("X.509");

			X509Certificate jceIssuer = (X509Certificate) jceFac.generateCertificate(new ByteArrayInputStream(certData));
			...
			X509Certificate bcIssuer = (X509Certificate) bcFac.generateCertificate(new ByteArrayInputStream(certData));
		...
		private void pemFileTest()
		{
			...
			Collection<Certificate> certs1 = (Collection<Certificate>)certFact.generateCertificates(this.GetType().getResourceAsStream("cert_chain.txt"));

			isTrue("certs wrong <cr><nl>", 2 == certs1.size());

			BufferedInputStream @in = new BufferedInputStream(this.GetType().getResourceAsStream("cert_chain.txt"));
		...
		private void invalidCRLs()
		{
			...
			Collection crls = certFact.generateCRLs(this.GetType().getResourceAsStream("cert_chain.txt"));
			isTrue("multi crl", crls.isEmpty());
			CRL crl = certFact.generateCRL(this.GetType().getResourceAsStream("cert_chain.txt"));
		...
		private void pemFileTestWithNl()
		{
			...
			Collection<Certificate> certs1 = (Collection<Certificate>)certFact.generateCertificates(this.GetType().getResourceAsStream("cert_chain_nl.txt"));

			isTrue("certs wrong <nl>", 2 == certs1.size());

			BufferedInputStream @in = new BufferedInputStream(this.GetType().getResourceAsStream("cert_chain_nl.txt"));
