...
namespace org.bouncycastle.jce.provider.test
{
	...
	/// </summary>
	public class BlockCipherTest : SimpleTest
	{
		private static Set<string> shortIvOkay = new HashSet<string>();

		static BlockCipherTest()
		{
			shortIvOkay.add("EAX");
			shortIvOkay.add("OCB");
			shortIvOkay.add("CCM");
		...
		{
			private readonly BlockCipherTest outerInstance;

			public FixedSecureRandom(BlockCipherTest outerInstance)
			{
		...
		public virtual void test(string algorithm, byte[] input, byte[] output)
		{
			Key key = null;
			KeyGenerator keyGen;
			SecureRandom rand;
			...
			try
			{
				...
				if (index > 0)
				{
					...
					mode = algorithm.Substring(index + 1, algorithm.LastIndexOf('/') - (index + 1));
				}
				else
				{
				...
				if (!@in.getAlgorithm().StartsWith(baseAlgorithm, StringComparison.Ordinal))
				{
					fail("wrong cipher returned!");
				}

				if (algorithm.StartsWith("RC2", StringComparison.Ordinal))
				{
					@out.init(Cipher.ENCRYPT_MODE, key, rc2Spec, rand);
				}
				else if (algorithm.StartsWith("RC5", StringComparison.Ordinal))
				{
					if (algorithm.StartsWith("RC5-64", StringComparison.Ordinal))
					{
			...
			{
				fail("" + algorithm + " failed encryption - expected " + StringHelper.NewString(Hex.encode(output)) + " got " + StringHelper.NewString(Hex.encode(bytes)));
			}
			...
			{
				fail("" + algorithm + " failed decryption - expected " + StringHelper.NewString(Hex.encode(input)) + " got " + StringHelper.NewString(Hex.encode(bytes)));
			}
			...
			try
			{
				...
				catch (ShortBufferException)
				{
					...
					{
						fail("doFinal " + algorithm + " failed decryption - expected " + StringHelper.NewString(Hex.encode(input)) + " got " + StringHelper.NewString(Hex.encode(out2)));
					}
				}
			}
			catch (TestFailedException e)
			...
			try
			{
				if (algorithm.IndexOf("CCM", StringComparison.Ordinal) < 0 && algorithm.IndexOf("Threefish", StringComparison.Ordinal) < 0 && algorithm.IndexOf("PGPCFB", StringComparison.Ordinal) < 0)
				{
					//
					...
					JavaSystem.arraycopy(input, 0, input2, 0, input.Length);
					JavaSystem.arraycopy(input, 0, input2, input.Length, input.Length);
					JavaSystem.arraycopy(input, 0, input2, input.Length * 2, input.Length);
					JavaSystem.arraycopy(input, 0, input2, input.Length * 3, input.Length);

					if (algorithm.IndexOf("GCM", StringComparison.Ordinal) > 0)
					{
						@out = Cipher.getInstance(algorithm, "BC");
						@out.init(Cipher.ENCRYPT_MODE, key, rand);
					}
					...
					catch (ShortBufferException)
					{
						...
						JavaSystem.arraycopy(input2, 0, out2, 0, out2.Length);

						int count = @out.update(out2, 0, out2.Length, out2, 0);

						if (!areEqual(out2, count, Arrays.copyOfRange(output2, 0, count)))
						{
							fail("update " + algorithm + " failed decryption - expected " + StringHelper.NewString(Hex.encode(output2)) + " got " + StringHelper.NewString(Hex.encode(out2)));
						}
					}
				}
		 ...
		 private void serialiseTest(string algorithm, byte[] input, byte[] output)
		 {
			 Key key = null;
			 KeyGenerator keyGen;
			 SecureRandom rand;
			 ...
			 if (algorithm.StartsWith("RC2", StringComparison.Ordinal))
			 {
				 if (baseAlgorithm.Equals(algorithm) || algorithm.IndexOf("ECB", StringComparison.Ordinal) > 0)
				 {
		...
		private void testExceptions()
		{
			...
			try
			{
				...
				skF.getKeySpec(null, ks.GetType());
			...
			try
			{
				...
				try
				{
					kg.init(int.MinValue, new SecureRandom());
			...
			try
			{
				byte[] rawDESKey = new byte[] {unchecked((byte)128), unchecked((byte)131), unchecked((byte)133), unchecked((byte)134), unchecked((byte)137), unchecked((byte)138), unchecked((byte)140), unchecked((byte)143)};
			...
			try
			{
				byte[] rawDESKey = new byte[] {-128, -125, -123, -122, -119, -118};
			...
			try
			{
				byte[] rawDESKey = new byte[] {-128, -125, -123, -122, -119, -118, -117, -115, -114};
		...
		public class PublicKeyAnonymousInnerClass : PublicKey
		{
			...
			public PublicKeyAnonymousInnerClass(BlockCipherTest outerInstance)
			{
				this.outerInstance = outerInstance;
			}
		...
		public class PrivateKeyAnonymousInnerClass : PrivateKey
		{
			...
			public PrivateKeyAnonymousInnerClass(BlockCipherTest outerInstance)
			{
				this.outerInstance = outerInstance;
			}
