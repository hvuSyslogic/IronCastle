...
namespace org.bouncycastle.jce.provider.test
{
	...
	using DERSequenceParser = org.bouncycastle.asn1.DERSequenceParser;
	...
	using PKCS12StoreParameter = org.bouncycastle.jcajce.PKCS12StoreParameter;
	...
	/// </para>
	/// </summary>
	public class PKCS12StoreTest : SimpleTest
	{
		...
		private void testGOSTStore()
		{
			...
			isEquals("mac alg not match", new AlgorithmIdentifier(CryptoProObjectIdentifiers_Fields.gostR3411, DERNull.INSTANCE), mData.getMac().getAlgorithmId());
		...
		public virtual void testPKCS12Store()
		{
			...
			if (!store.entryInstanceOf("cert", typeof(KeyStore.TrustedCertificateEntry)))
			{
			...
			if (store.entryInstanceOf("cert", typeof(KeyStore.PrivateKeyEntry)))
			{
		...
		private void basicStoreTest(PrivateKey privKey, Certificate[] chain, string type)
		{
			...
			{
				if (c[0] is X509CertificateObject)
				{
			...
			if (b1.getBagAttribute(PKCSObjectIdentifiers_Fields.pkcs_9_at_friendlyName) != null)
			{
				DERBMPString name = (DERBMPString)b1.getBagAttribute(PKCSObjectIdentifiers_Fields.pkcs_9_at_friendlyName);
			...
			if (b1.getBagAttribute(PKCSObjectIdentifiers_Fields.pkcs_9_at_localKeyId) != null)
			{
				ASN1OctetString id = (ASN1OctetString)b1.getBagAttribute(PKCSObjectIdentifiers_Fields.pkcs_9_at_localKeyId);

				if (!id.Equals(b2.getBagAttribute(PKCSObjectIdentifiers_Fields.pkcs_9_at_localKeyId)))
				{
			...
			if (!encInfo.getEncryptionAlgorithm().getAlgorithm().Equals(PKCSObjectIdentifiers_Fields.pbeWithSHAAnd3_KeyTripleDES_CBC))
			{
			...
			if (type.EndsWith("3DES", StringComparison.Ordinal))
			{
				if (!cb.getEncryptionAlgorithm().getAlgorithm().Equals(PKCSObjectIdentifiers_Fields.pbeWithSHAAnd3_KeyTripleDES_CBC))
				{
					fail("expected 3DES found: " + cb.getEncryptionAlgorithm().getAlgorithm());
				}
			}
			else if (type.EndsWith("40RC2", StringComparison.Ordinal))
			{
				if (!cb.getEncryptionAlgorithm().getAlgorithm().Equals(PKCSObjectIdentifiers_Fields.pbeWithSHAAnd40BitRC2_CBC))
				{
		...
		private void testNoExtraLocalKeyID(byte[] store1data)
		{
			...
			JavaSystem.arraycopy(chain1, 0, chain2, 1, chain1.Length);

			chain2[0] = createCert(newPair.getPublic(), k1, "subject@bouncycastle.org", "extra@bouncycaste.org");

			if (((PKCS12BagAttributeCarrier)chain1[0]).getBagAttribute(PKCSObjectIdentifiers_Fields.pkcs_9_at_localKeyId) == null)
			{
			...
			if (((PKCS12BagAttributeCarrier)chain2[1]).getBagAttribute(PKCSObjectIdentifiers_Fields.pkcs_9_at_localKeyId) != null)
			{
		...
		private void testIterationCount()
		{
			...
			{
				isTrue(e.Message.EndsWith("iteration count 2000 greater than 10"));
			}
		...
		private void testBCFKSLoad()
		{
			...
			{
				isTrue("malformed sequence".Equals(e.Message));
			}
			...
			{
				isTrue("illegal object in getInstance: org.bouncycastle.asn1.DLSequence".Equals(e.Message));
			}
		}
