namespace org.bouncycastle.jce.provider.test
{
	...
	public class AEADTest : SimpleTest
	{
		...
		{
			bool aeadAvailable = false;
			try
			{
				this.GetType().getClassLoader().loadClass("javax.crypto.spec.GCMParameterSpec");
				aeadAvailable = true;
			}
			...
			{
				JavaSystem.err.println("GCM AEADTests disabled due to JDK");
			}
			testTampering(aeadAvailable);
		}

		private void testTampering(bool aeadAvailable)
		{
			Cipher eax = Cipher.getInstance("AES/EAX/NoPadding", "BC");
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final javax.crypto.spec.SecretKeySpec key = new javax.crypto.spec.SecretKeySpec(new byte[eax.getBlockSize()], eax.getAlgorithm());
			SecretKeySpec key = new SecretKeySpec(new byte[eax.getBlockSize()], eax.getAlgorithm());
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final javax.crypto.spec.IvParameterSpec iv = new javax.crypto.spec.IvParameterSpec(new byte[eax.getBlockSize()]);
			...
			catch (BadPaddingException e)
			{
				if (aeadAvailable)
				{
					if (!e.GetType().getName().Equals("javax.crypto.AEADBadTagException"))
					{
		...
		private void testAEADParameterSpec(byte[] K, byte[] N, byte[] A, byte[] P, byte[] C)
		{
			Cipher eax = Cipher.getInstance("AES/EAX/NoPadding", "BC");
			SecretKeySpec key = new SecretKeySpec(K, "AES");
			...
			AEADParameterSpec cSpec = algParams.getParameterSpec(typeof(AEADParameterSpec));
			...
			cSpec = aeadParams.getParameterSpec(typeof(AEADParameterSpec));
			if (!Arrays.areEqual(spec.getIV(), cSpec.getNonce()) || spec.getMacSizeInBits() != cSpec.getMacSizeInBits() || cSpec.getAssociatedData() != null)
			{
				TestCase.fail("parameters mismatch");
			}
		...
		private void testGCMParameterSpec(byte[] K, byte[] N, byte[] A, byte[] P, byte[] C)
		{
			Cipher eax = Cipher.getInstance("AES/EAX/NoPadding", "BC");
			SecretKeySpec key = new SecretKeySpec(K, "AES");
		...
		private void testGCMParameterSpecWithMultipleUpdates(byte[] K, byte[] N, byte[] A, byte[] P, byte[] C)
		{
			Cipher eax = Cipher.getInstance("AES/EAX/NoPadding", "BC");
			SecretKeySpec key = new SecretKeySpec(K, "AES");
		...
		private void testGCMGeneric(byte[] K, byte[] N, byte[] A, byte[] P, byte[] C)
		{
			...
			GCMParameterSpec gcmSpec = algParams.getParameterSpec(typeof(GCMParameterSpec));
		...
		private void testRepeatedGCMWithSpec(byte[] K, byte[] N, byte[] A, byte[] P, byte[] C)
		{
			Cipher eax = Cipher.getInstance("AES/GCM/NoPadding", "BC");
			SecretKeySpec key = new SecretKeySpec(K, "AES");
			GCMParameterSpec spec = new GCMParameterSpec(128, N);
