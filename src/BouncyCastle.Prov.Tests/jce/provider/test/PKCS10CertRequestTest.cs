...
namespace org.bouncycastle.jce.provider.test
{
	...
	/// 
	public class PKCS10CertRequestTest : SimpleTest
	{
		...
		private void generationTest(int keySize, string keyName, string sigName, string provider)
		{
			...
			PKCS10CertificationRequest req1 = new PKCS10CertificationRequest(sigName, subject, kp.getPublic(), null, kp.getPrivate(), provider);
		...
		private void createECRequest(string algorithm, ASN1ObjectIdentifier algOid, ASN1ObjectIdentifier curveOid)
		{
			...
			PKCS10CertificationRequest req = new PKCS10CertificationRequest(algorithm, new X509Name("CN=XXX"), pubKey, null, privKey);
			if (!req.verify())
			{
			...
			req = new PKCS10CertificationRequest(algorithm, new X509Name("CN=XXX"), pubKey, null, privKey);
			if (!req.verify())
			{
		...
		private void createPSSTest(string algorithm)
		{
			...
			if (!req.getSignatureAlgorithm().getAlgorithm().Equals(PKCSObjectIdentifiers_Fields.id_RSASSA_PSS))
			{
		...
		private void nullPointerTest()
		{
			...
			Attribute attribute = new Attribute(PKCSObjectIdentifiers_Fields.pkcs_9_at_extensionRequest, new DERSet(new X509Extensions(oids, values)));

			PKCS10CertificationRequest p1 = new PKCS10CertificationRequest("SHA1WithRSA", new X509Principal("cn=csr"), pair.getPublic(), new DERSet(attribute), pair.getPrivate(), "BC");
			PKCS10CertificationRequest p2 = new PKCS10CertificationRequest("SHA1WithRSA", new X509Principal("cn=csr"), pair.getPublic(), new DERSet(attribute), pair.getPrivate(), "BC");
		...
		public override void performTest()
		{
			...
			req = new PKCS10CertificationRequest("ECDSAWITHSHA1", new X509Name("CN=XXX"), kp.getPublic(), null, kp.getPrivate());
			if (!req.verify())
			{
				fail("Failed verify check EC.");
			}
