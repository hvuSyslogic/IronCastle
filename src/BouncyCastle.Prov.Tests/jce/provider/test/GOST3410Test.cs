...
namespace org.bouncycastle.jce.provider.test
{
	...
	public class GOST3410Test : SimpleTest
	{
		private static byte[] ecgostData = new byte[] {1, 2, 3, 4, 5, 6, 7, 8, 9, 0};
		...
		private void ecGOST3410Test()
		{
			...
			ECCurve curve = new ECCurve.Fp(mod_p, new BigInteger("7"), new BigInteger("43308876546767276905765904595650931995942111794451039583252968842033849580414"), mod_q, ECConstants_Fields.ONE);

			ECParameterSpec spec = new ECParameterSpec(curve, curve.createPoint(new BigInteger("2"), new BigInteger("4018974056539037503335449422937059775635739389905545080690979365213431566280")), mod_q, ECConstants_Fields.ONE);
			...
			ECPublicKeySpec pubKey = new ECPublicKeySpec(curve.createPoint(new BigInteger("57520216126176808443631405023338071176630104906313632182896741342206604859403"), new BigInteger("17614944419213781543809391949654080031942662045363639260709847859438286763994")), spec);
	...
	//                    new BigInteger("57896044618658097711785492504343953927082934583725450622380973592137631069619")); // q
		...
		private void ecGOST34102012256Test()
		{
			...
			param = new ECPublicKeyParameters(ecPublicKey.getQ(), new ECDomainParameters(spec.getCurve(), spec.getG(), spec.getN()));
		...
		private void ecGOST2012NameCurveGenerationTest()
		{
			...
			AlgorithmIdentifier expectedAlgId = new AlgorithmIdentifier(RosstandartObjectIdentifiers_Fields.id_tc26_gost_3410_12_256, new GOST3410PublicKeyAlgParameters(RosstandartObjectIdentifiers_Fields.id_tc26_gost_3410_12_256_paramSetA, RosstandartObjectIdentifiers_Fields.id_tc26_gost_3411_12_256));
			...
			expectedAlgId = new AlgorithmIdentifier(RosstandartObjectIdentifiers_Fields.id_tc26_gost_3410_12_512, new GOST3410PublicKeyAlgParameters(RosstandartObjectIdentifiers_Fields.id_tc26_gost_3410_12_512_paramSetA, RosstandartObjectIdentifiers_Fields.id_tc26_gost_3411_12_512));
			...
			expectedAlgId = new AlgorithmIdentifier(RosstandartObjectIdentifiers_Fields.id_tc26_gost_3410_12_512, new GOST3410PublicKeyAlgParameters(RosstandartObjectIdentifiers_Fields.id_tc26_gost_3410_12_512_paramSetB, RosstandartObjectIdentifiers_Fields.id_tc26_gost_3411_12_512));
			...
			expectedAlgId = new AlgorithmIdentifier(RosstandartObjectIdentifiers_Fields.id_tc26_gost_3410_12_512, new GOST3410PublicKeyAlgParameters(RosstandartObjectIdentifiers_Fields.id_tc26_gost_3410_12_512_paramSetC, RosstandartObjectIdentifiers_Fields.id_tc26_gost_3411_12_512));
		...
		private void ecGOST34102012512Test()
		{
			...
			ECCurve curve = new ECCurve.Fp(mod_p, new BigInteger("7"), new BigInteger("1518655069210828534508950034714043154928747527740206436194018823352809982443793732829756914785974674866041605397883677596626326413990136959047435811826396"), mod_q, ECConstants_Fields.ONE);

			ECParameterSpec spec = new ECParameterSpec(curve, curve.createPoint(new BigInteger("1928356944067022849399309401243137598997786635459507974357075491307766592685835441065557681003184874819658004903212332884252335830250729527632383493573274"), new BigInteger("2288728693371972859970012155529478416353562327329506180314497425931102860301572814141997072271708807066593850650334152381857347798885864807605098724013854")), mod_q, ECConstants_Fields.ONE);

			ECPrivateKeySpec priKey = new ECPrivateKeySpec(new BigInteger("610081804136373098219538153239847583006845519069531562982388135354890606301782255383608393423372379057665527595116827307025046458837440766121180466875860"), spec);

			ECPublicKeySpec pubKey = new ECPublicKeySpec(curve.createPoint(new BigInteger("909546853002536596556690768669830310006929272546556281596372965370312498563182320436892870052842808608262832456858223580713780290717986855863433431150561"), new BigInteger("2921457203374425620632449734248415455640700823559488705164895837509539134297327397380287741428246088626609329139441895016863758984106326600572476822372076")), spec);
	...
	//                }
			...
			GOST3410ParameterSpec gost3410P = new GOST3410ParameterSpec(CryptoProObjectIdentifiers_Fields.gostR3410_94_CryptoPro_B.getId());
