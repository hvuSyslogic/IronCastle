...
namespace org.bouncycastle.jce.provider.test
{
	...
	public class ElGamalTest : SimpleTest
	{
		...
		private void testGP(int size, int privateValueSize, BigInteger g, BigInteger p)
		{
			...
			if (keyPair.getPublic().GetHashCode() != pubKey.GetHashCode())
			{
			...
			if (keyPair.getPrivate().GetHashCode() != privKey.GetHashCode())
			{
				fail("hashCode test failed");
			}
		...
		private void testDefault(int privateValueSize, BigInteger g, BigInteger p)
		{
			...
			(new BouncyCastleProvider()).setParameter(ConfigurableProvider_Fields.DH_DEFAULT_PARAMS, elParams);
